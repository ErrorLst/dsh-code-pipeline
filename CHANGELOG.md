...
本文件保存全部历史版本说明（新→旧）。README 只描述**当前**行为；
每个版本的改动、根因与实测证据都在这里。

版本与 `package.json` 的 `version` 同步；预设改动随包发布，安装后由 dsh 按声明加载。
...
- 本版本只支持 **dsh ≥ 0.1.7-alpha.1**（`engines.dsh` 早已如此声明），并删除全部 ≤0.1.6 兼容代码：`$DSH_HOME/.agent-presets` 自动安装 / 指纹同步 / 备份、`settings.installSection` 回退、`settings.yaml` 旧节改名与 `.imported` 一次性导入。
- **从 0.4.x 直升 0.6.0 的用户请先升级到 0.5.0**，由 0.5.0 完成设置从 `settings.yaml` 到插件 Config volatile 字段的一次性迁移。
- 运行时不再依赖未声明的 `yaml`（旧设置导入路径已删除）；`yaml` 移入 `devDependencies`，只供测试与检查脚本使用。
- `lib/index.js` 不再导出 `presetBundleDigest` / `presetSyncPath` / `reconcileCompactionRow`。前两者随预设指纹同步删除；`reconcileCompactionRow` 是把比例写进**已安装组合文件**的文本对账器，生产调用方（`reconcileInstalledCompaction`）删除后成为死代码，连同其 5 个文本处理助手与测试 F 块一并移除。

### 新增

- **宿主自检**：`lib/host-contract.js` 列出插件用到的全部宿主面（8 个服务 + 10 个事件），`lib/host.js` 提供 `getService` / `onHost` / `probeHost`。启动时一条 INFO 汇总；服务在 apply 之后才注册的缺席只记 INFO，只有「在场但缺必需方法」（真正的 API 漂移）才 WARN；`GET /dsh-code-pipeline/status` 新增 `host` 字段（逐面探测结果 + 注入健康 `matched` / `injected` / `failed`）。缺服务从「静默失效」变成可诊断状态。
- **预设漂移检查**：`npm run check:preset`（`scripts/check-preset-upstream.mjs`）比对包内预设与已安装 dsh 的内置 `ptc` 预设：
  - 包名在当前 dsh 树中不可解析 → 错误（会让**整份预设挂载失败**，即 `workflow-ptc` 那一类事故）；
  - 上游**启用**而本地缺失的行 → 错误（功能丢失）；
  - 上游已禁用的缺失行、`disabled` 标志差异 → 信息。
  - `npm run verify` = smoke + 该检查，作为 dsh 升级后的第一步。
- README 新增「兼容与自检」与「dsh 升级后」两节。

### 安全（dsh 不会被插件带坏）

- **`agent/request` 的思考等级注入**此前在 `next()` 之后读设置且无 try/catch —— 宿主一旦改变 volatile 引用形状，异常会抛进模型请求链、让**整轮会话失败**。现在失败只告警，并返回**原样 config**。
- **`subagent/end`** 的解析 / 校验 / 遥测整块加 try/catch（它跑在宿主的 settle 路径上）；释放名额与相位推进在任何情况下都照常完成。
- 所有宿主事件注册统一走 `onHost`：注册失败只告警并继续，绝不让 `apply`（或宿主 boot）因一个事件改名而中断。
- `isRootAgent` 在无法判定归属时**不注入**——宁可漏注入，也不把阶段工具泄漏进子代理自有层（宿主 `tools.restrict` 不过滤自有层）。
- `reconcileCompactionPlugins` 无变化时返回**原引用**（旧实现用 `Array.map` 恒重建整棵树，而调用方会把它写回宿主 profile patch）；写入前校验结构，失败只告警。
- 压缩阈值同步链显式 `.catch`：后台写入失败只留一条日志，不产生未处理 rejection。
- `subagent/start` 与 `agent/disposed` 处理器同样包上 try/catch（分别跑在宿主的激活 / 销毁路径上）；设置页路由注册失败的重试定时器 `unref`，不再拖住进程退出。

### 设置页去口水

- 新增 `HelpText`：常显一句（每条 ≤80 字符）+ 原生 `<details>说明</details>` 折叠长解释。
- 删除重复文案：三处阶段提示里的「（deepseek-official / deepseek-flash 默认）」（下拉框里已经写着）、intro 与字段里重复的并发 / 墙钟说明；动态状态行改短（`运行 N / 上限 M`、`预算 N 分钟`）。
- `Config` 各字段的 `description` 压到一句话；`package.json` 的 `description` 从约 4300 字符压到一句话（插件管理页直接渲染它）。

### 文档

- README 963 → 约 250 行，只描述当前行为；全部历史条目原样迁入本文件。
- `docs/optimization-plan.zh.md` → `docs/design-notes.zh.md`（历史设计稿，已纳入 git）。

### 测试

- 断言 234 → **239**：新增 K（宿主安全与契约：契约清单 vs 源码字面量、缺服务降级、`agent/request` 原样返回、`subagent/end` 抗畸形载荷、profile 树引用契约、`configEditor.edit` 失败只告警）、L（漂移检查纯函数 + `build:preset` 未过期 + 子路径包根）、M（设置卡片文案预算）共 21 项；删除 F / G 两块（分别测试已删除的 `.agent-presets` 文本对账器与 `installSection` 写盘链，共 16 项）。测试不再需要临时 `DSH_HOME` 与 `.agent-presets`。

## 0.5.0 — 适配 dsh 0.1.7：预设声明化 + 设置迁移到 volatile Config

- 预设改为随包的声明行 `preset/code-pipeline/cordis.patch.yml`（`@deepseek-ai/dsh-agent-preset`），不再写 `$DSH_HOME/.agent-presets`（0.1.7 已不再读取该目录）。
- 设置从 `settings.yaml` 的 `code-pipeline` 节迁移到插件 Config 的 volatile 字段：值持久化到 profile 的 Cordis patch（`dsh-code-pipeline` 条目），浏览器端经 `ctx.configForms` 读写。首次启动会把 `settings.yaml` 的 `code-pipeline:` 节改名为 `dsh-code-pipeline:` 交给宿主一次性导入。
- 压缩触发比例 `compactionThresholdRatio` 写进预设声明的 `compaction-basic` 行（经 `configEditor` 改 profile patch），对新挂载的代理生效。
...
- **0.4.7（把「读有下限」告知 agent：预设 / 阶段 persona / 派发块实时数值注入；默认下限 500 → 200）**：
  - **问题**：机械拓宽只保证「单次读够大」，模型不知道有下限，仍会主动写出一串小窗读——实测同一个 `run_code` 程序里对同一文件先读 10~20 行、再读 50~60 行，窗口互相重叠，程序内重复抓取同一片内容。
  - **做法（三处告知注入）**：① 预设 persona 的 `### Step economy` 新增「Reads carry a FLOOR（default 200）：低于下限的 `limit` 会被自动拓宽——别把一个文件切成连续小窗读；整文件或 ≥ 下限的范围一个区域只读一次，拿到的窗口比要的宽是正常的」；② 三段阶段 persona 的 TOOL GOTCHAS 同步「NEVER chunk one file into successive small reads」（同一文件的多次 sub-floor 读只是重复抓重叠窗口）；③ `renderFilesBlock` 新增第二参数 `readFloor`，files 派发块（阶段派发与 `pipeline_followup` 共用）注入**实时**下限值「Your READ FLOOR is N lines …」，设置页改完下一次派发即生效，下限 0 时改注「拓宽已关闭」。
  - **做法（默认 200）**：`READ_WIDEN_DEFAULT_LINES` 500 → 200（200 足以拦下「10~60 行小窗试探」；想要整窗调 2000）。schema / 客户端兜底 / 提示文案同步。
  - **预设改动**：`agent.cordis.yml` 内容变化 → 升级启动时指纹自动同步覆盖已安装预设（覆盖前留 temp 备份）。
...
- **0.4.6（修复「read 读取窗口下限」保存无效 + 默认下限 2000 → 500）**：
  - **问题①（保存无效）**：设置页改完下限点保存，重开仍是旧值。根因是 `lib/client.js` 的 `save` useCallback **依赖数组漏了 `readWidenMinLines`**——闭包捕获的是初始渲染值（兜底 2000），点保存永远把旧值写回 `settings.yaml`（`stages` / `followupMode` / `compactionThresholdRatio` 都在依赖里，唯独新加的字段漏了，所以只有它失效）。
  - **问题②（默认过激）**：0.4.5 的默认下限 2000（= 工具上限，一律整窗）对超大文件一次拉满，实测反馈偏激进。
  - **做法**：`save` 依赖数组补上 `readWidenMinLines`（W12 回归断言钉死）；默认 `READ_WIDEN_DEFAULT_LINES` 2000 → 500（覆盖典型源码文件大半，又不对超大文件一次拉满；`2000` = 一律整窗，`0` = 关闭），schema / 客户端兜底 / 提示文案 / 冒烟断言同步按常量断言。**已保存过 2000 的安装：显式存储值优先于默认，需在设置页重存一次（或删掉 `settings.yaml` 里 `code-pipeline.readWidenMinLines` 那行以继承新默认）。**
  - **验证**：冒烟 W2/W3/W9b/W10 改按 `plugin.READ_WIDEN_DEFAULT_LINES` / `READ_TOOL_MAX_LINES` 断言（默认值不再硬编码）；新增 W12（save 依赖数组包含 readWidenMinLines + 客户端兜底与插件默认一致）。断言数 **222 → 224**、0 failure。
  - 版本 0.4.5 → 0.4.6。
