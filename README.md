...
`run_code` 程序里对同一文件先读 10~20 行、再读 50~60 行，窗口互相重叠）。所以除机制外，
插件还把「读有下限」写进 agent 的常识，三处注入：

- **预设 persona（root 代理）**：`### Step economy` 新增一条——读有下限（默认 200），
...
- **files 派发块（`renderFilesBlock`）**：注入**实时**下限值（「Your READ FLOOR is N
