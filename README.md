...
> **read 读取窗口下限（`readWidenMinLines`）默认 500**（0.4.6 起；0.4.5 曾为
> 2000）：本预设代理发出的 read，`limit` 低于该值会在执行前被就地拓宽到该值，
> 超过 2000 的治愈为 2000；`2000` = 一律整窗，`0` = 关闭。见「read 读取窗口拓宽
> （一次多读）」。
...
- `limit` 低于下限 → 拓宽到下限（默认 **500**；调到 2000 = read 工具上限，等价于整窗 / 省略 `limit`）；
...
默认 500（0.4.5 曾为 2000，实测反馈后调低）：本预设所有代理都是 PTC（Code Mode），
`read` 只在 `run_code` 程序内部发生——**嵌套工具结果只进程序、不进模型历史**，拓宽
本身零 token 成本；500 已覆盖典型源码文件的大半，把「几十行几十行翻」的步数砍掉一个
数量级，又不对超大文件一次拉满，想「一律整窗」调到 2000 即可。结果自带行号与
...
默认 `500`；`2000` = 一律整窗；`0` = 关闭拓宽。改动立即对后续 read 生效，无需重启。

**验证**：冒烟 W1–W12（小 limit 拓宽 / 超上限治愈 / 下限可调可关 / 非本预设代理
