...
/**
 * plan 的并行预算说明：告诉规划者这个会话**最多还能创建几个实现者**，让它把 Workstreams
 * 表切进预算之内、并为超出的部分排出**串行顺序**。
 *
 * 根因（0.3.8）：plan 切了 5 个工作流，而 impl 的创建上限是 2 → 后 3 个只能被塞进「已经做过
 * 另一个 workstream」的冷 child 里（无法压缩）→ 实测那一轮 63 步 / 9.7M tokens，每步重发约 152k。
 * 与其在派发时抢救，不如让 plan 一开始就知道预算。
 *
 * @param budget - the {@link stageParallelBudget} result for the impl stage.
 * @returns the model-facing block body (no heading).
 */
function renderParallelismBudget(budget) {
  if (budget === undefined || budget.unlimited === true) {
    return "No plugin-side creation limit is configured for this session (Settings → 代码流水线 → 最大并发 = 0). The host still caps how many continuable children may be ALIVE at once (default 8), so keep the split to a handful of genuinely independent workstreams.";
  }
  const head = "This session may create at most " + String(budget.limit) + " implementer (`subagent_impl`) child(ren) for this task"
    + " (Settings → 代码流水线 → 最大并发; already created: " + String(budget.used) + ", remaining: " + String(budget.remaining) + "). ";
  if (budget.remaining <= 0) {
    return head + "REMAINING = 0: no new implementer can be created at all. Plan the change as an addition to the implementer(s) that already exist (the orchestrator delivers it with pipeline_followup plus a files list), and say explicitly if the task genuinely needs a new child.";
  }
  return head + "Your `## Workstreams` map MUST contain AT MOST " + String(budget.remaining) + " workstream(s). "
    + "If your natural decomposition yields more, MERGE them down to " + String(budget.remaining) + " and give the map an explicit SERIAL ORDER: which workstream runs first, which must wait for which, and what each later one inherits. "
    + "Do NOT leave the orchestrator to squeeze the surplus into a reused implementer — a settled child cannot be compacted, so a later workstream handed to an unrelated implementer re-sends that child's entire history on every step (measured: 63 steps at ~150k re-sent tokens per step, ~10M extra tokens for one round). "
    + (budget.remaining === 1
      ? "The budget is 1: say so and plan the work as a single SERIAL sequence with ordered steps (one implementer does them in order), not as parallel workstreams."
      : "When you merge, say which workstreams were merged and why.");
}
...
async function materializeDispatchMessage(stage, args, promptText, cfg, parallelBudget) {
  ...
  // 只有 plan 需要预算：它的 Workstreams 表决定后面会创建几个 impl child。
  if (stage.key === "plan" && parallelBudget !== undefined) {
    parts.push("\n\n**parallelismBudget**\n" + renderParallelismBudget(parallelBudget));
  }
...
function stageConcurrencyReached(stage, running, starting, limit) {
  ...
  return new Error(
    ...
    + "If the work you are about to hand over is a DIFFERENT workstream, prefer a child that is still RUNNING (steering it needs no slot) and ALWAYS send the candidate list in `files` — a cold child that has to rediscover the file list pays for it on every step of the round.",
  );
}
...
function stageCreationCapReached(stage, used, limit, available) {
  ...
  return new Error(
    ...
    + "CROSS-WORKSTREAM REUSE HAS A PRICE — read this before handing over work that is a DIFFERENT workstream (different files, unrelated to what that child just did). A settled (cold) child cannot be compacted, so EVERY step of the new round re-sends its entire accumulated history. Measured on a real run: one cross-workstream reuse ran 63 steps at ~150k re-sent tokens per step (~10M extra tokens for that single round), and it began by spending 7 steps / 15 reads rediscovering a file list the orchestrator already had. Order of preference: (a) if one of the reusable children is still RUNNING, deliver to it — a live message reuses its slot and does not re-wake it; (b) always send the real candidate list in `files` so the child never re-derives it; (c) if the plan declared MORE workstreams than this cap allows, that plan was always going to be squeezed — MERGE the remaining workstreams into ONE child deliberately and let them run serially there, and say so; (d) otherwise reuse and accept the cost. Dispatch a fresh child only while this session still has a creation slot.\n"
    + "Your next step — one of these three:\n"
    + "  1) REUSE an existing child: call pipeline_followup with child = an exact id from the list above, carrying your new/updated requirement AND a `files` list (use the single character `-` only for a correction inside the workstream that child is already working on).\n"
    + "  2) If that child's accumulated history actively interferes with the new work (the same task family but the direction changed, a new requirement reverses a conclusion it contributed, or the diff it wrote was discarded wholesale), you cannot compact it away — a settled (cold) child cannot be compacted and waking it does not help — so pick (b) / (c) / (d) from the price paragraph above and say which you chose.\n"
    + "  3) If every reusable child is currently running, do NOT wait idly when the work is an independent workstream: a RUNNING child can be steered right now (pipeline_followup to it) and that reuses its slot; wait only when you specifically need its current turn to finish first.",
  );
}

/**
 * 供 plan 阶段参考的并行预算：这个父会话在某个阶段**还能创建几个新 child**。
 * 与创建上限准入同源（本进程台账 ∪ 最近一次成功观测的高水位），但**不做任何 I/O**——
 * 它只是给规划者的提示，真正的准入仍在阶段工具的 execute 里判定（那里的口径更严：
 * 还会异步核对宿主 listChildren）。limit = 0 表示本插件不限制。
 *
 * @param stageKey - the stage whose creations are being budgeted.
 * @param parentId - the delegating session id.
 * @param source - the live settings source.
 * @returns the budget the plan should design its workstream split against.
 */
function stageParallelBudget(stageKey, parentId, source) {
  const limit = normalizeConcurrency(resolveStages(source)[stageKey].maxConcurrency);
  if (limit <= 0) return { unlimited: true, limit: 0, used: 0, remaining: 0 };
  const used = Math.max(
    ledgerStageChildren(parentId, stageKey).length,
    observedCreated(parentId, stageKey)?.count ?? 0,
  );
...
function buildStageToolDef(ctx, stage, source, cfg) {
  return {
    ...
    async execute(args, exec) {
      ...
      // plan 的 Workstreams 表决定后面能创建几个 impl child，所以派发时就把它这一轮的
      // 创建预算一起告诉它（0.3.9：plan 切 5 个、上限 2 → 后 3 个被塞进冷 child，一轮 63 步）。
      const parallelBudget = stageParallelBudget("impl", parent.id, source);
      const { fullPrompt } = await materializeDispatchMessage(stage, args, promptText, cfg, parallelBudget);
...
function buildFollowupToolDef(ctx, source, cfg) {
  return {
    ...
    description:
      ...
      "FILES IS NOT OPTIONAL: pass the REAL list whenever this round touches a file set the child has NOT been working on — a child reused for a DIFFERENT workstream otherwise re-derives the whole list step by step (an observed reuse burned 7 steps / 15 reads doing exactly that). For a small correction inside the workstream the child is already working on, pass the single character \"-\". " +
    ...
    parameters: {
      ...
      properties: {
        ...
        files: {
          type: "string",
          description: "REQUIRED. The paths THIS round must read, newline-separated — the plugin renders them into a \"read every path in ONE run_code program\" instruction, exactly like a stage dispatch. Give the real list whenever the round touches files the child has not been working on (a cross-workstream reuse: without it the child spends steps rediscovering the list). For a small correction inside the workstream the child is already working on, pass the single character \"-\" to declare that no candidate list applies.",
        },
        issues: {
          type: "array",
          items: ISSUE_SCHEMA,
