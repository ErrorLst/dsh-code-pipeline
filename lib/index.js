...
const DEFAULT_STAGES = {
  plan: { provider: "deepseek-official", model: "deepseek-v4-flash", reasoningEffort: "", maxConcurrency: 0 },
  impl: { provider: "deepseek-official", model: "deepseek-v4-flash", reasoningEffort: "", maxConcurrency: 0 },
  review: { provider: "deepseek-official", model: "deepseek-v4-flash", reasoningEffort: "", maxConcurrency: 0 },
};
...
"Pipeline PLAN stage. CONTRACT: params = prompt|task (task text; one of them required) + description (display label, optional) + context? + constraints? (both merged into the child's prompt, never dropped). Unknown keys are rejected. BACKGROUND-ONLY: returns { kind: 'continuable', subagentId } immediately; the runtime notifies this session when it settles — the child's final reply arrives in this session as a settled notice; read the plan from that notice. PLANNING ONLY: never use it to review/audit/verify/approve code (that is subagent_review's job). PRESENTATION RULE: print the plan as a normal Markdown reply and END the turn — NEVER call ask_user_question to confirm it. The stage model is configured in Settings → 代码流水线. The whole dispatch message (prompt + all stage inputs) is written to a SINGLE temp file automatically — the child reads that one file with the read tool; never inline large material into these fields (inline generation is truncated at the model output limit)." +
      ...
      " CONCURRENCY: each stage has its own concurrent-child cap (Settings → 代码流水线 → 最大并发; 0 = unlimited). A call rejected for that cap is TRANSIENT, not a stage failure — do NOT stop the task: wait for a settled notice, then dispatch the remaining targets. When the targets are independent, dispatch them in PARALLEL in ONE program (Promise.all) so they work simultaneously and the work finishes sooner.",
  },
  {
    key: "impl",
...
const STAGE_SCHEMA = Schema.object({
  ...
  maxConcurrency: Schema.number()
    ...
    .max(64)
    .default(0)
    .description("该阶段最大同时运行子代理数（同一父会话内）；0 = 不限制。仍受宿主每个 run_code 程序最多 10 个并行子调用的上限约束"),
});
 ...
 * UNAVAILABLE_GUIDANCE（那会让主代理终止整个任务）。这里明确要求等待完成通知
 * 后再派发剩余目标。
 */
function stageConcurrencyReached(stage, running, limit) {
  return new Error(
    `${stage.toolName}: stage concurrency limit reached — ${running} child(ren) of stage "${stage.label}" are already running for this agent (limit ${limit}; Settings → 代码流水线 → 最大并发). `
    + "This is NOT a stage failure and NOT a reason to stop the task: wait for a settled notice, then dispatch the remaining targets in a later step. "
    + "To change the task of an already-running child, use pipeline_followup instead of starting another.",
  );
}
...
function buildStageToolDef(ctx, stage, source, cfg) {
  return {
    ...
    async execute(args, exec) {
      ...
      let reserved = false;
      if (limit > 0) {
        const before = activeStageCount(parent.id, stage.key) + pendingStageCount(parent.id, stage.key);
        if (before >= limit) throw stageConcurrencyReached(stage, before, limit);
        ...
        try {
          ...
          const inFlight = running + pendingStageCount(parent.id, stage.key) - 1;
          if (inFlight >= limit) throw stageConcurrencyReached(stage, inFlight, limit);
        } catch (error) {
...
function buildFollowupToolDef(ctx, source, cfg) {
  return {
    ...
    async execute(args, exec) {
      ...
      return { ok: true, childId, stage: entry?.stage ?? "unknown", ...(messageId === undefined ? {} : { messageId }) };
    },
  };
}
/**
...
export async function apply(ctx, config = {}) {
  ...
  // 设置页的并发状态端：各阶段当前运行数 / 启动中 / 上限（只读，跨父会话汇总）。
  const statusHandler = async (_req, res) => {
    ...
    for (const stage of STAGES) {
      ...
      stages[stage.key] = {
        ...
        limit: normalizeConcurrency(resolveStages(source)[stage.key].maxConcurrency),
      };
    }
