...
//   - 设置持久化：dsh 0.1.7 起是插件 Config 的 volatile 字段（写入 profile 的
//     Cordis patch，客户端经 configForms 读写）；0.1.6 及更早回退 settings.yaml 的
//     code-pipeline 节（该回退只在旧宿主上生效）。
//
...
const STAGE_SCHEMA = Schema.object({
  ...
  maxConcurrency: Schema.number()
    ...
    .max(64)
    .default(0)
    .description("同时运行的并发上限：同一父会话内该阶段允许「同时在跑」的子代理数；0 = 本插件不再限制（默认）。没有「已创建总量」上限——新工作流一律派发新子代理；只有同一工作流的续用才走 pipeline_followup。仍受宿主每个 run_code 程序最多 10 个并行子调用的上限约束；dsh 0.1.6-alpha.2 起还受宿主「每个会话同时存活的 continuable 子代理数」上限约束（subagent.maxActiveSubagents，默认 8，可在 设置 → 内置插件 → 子代理 调大）——该上限在子代理结束时释放"),
...
// ── 预设自动安装 / 自动同步（参照其它 bundle 插件的一键安装体验）──────────────
// `dsh plugin add` 只完成依赖 + bundle 登记；code-pipeline 预设文件是本插件的
// 交付物，这里在启动时保证 $DSH_HOME/.agent-presets/ 与包内 preset/ 一致：
// 目录缺失 → 拷贝；已存在 → 比对安装目录 .dsh-bundle.json 记下的「上次同步指纹」，
// 指纹变了（包内预设随插件升级而变化）就自动覆盖并把旧副本备份到 temp，
// 指纹一致则保留用户在两次升级之间做的本地改动。
...
/** 比较前抹掉设置页写进预设的压缩比例，避免「用户改过阈值」被误报成「预设落后」。 */
function stripCompactionRatios(text) {
  return text
    .replace(/^(\s*thresholdRatio:\s*)\S+(\s*(?:#.*)?)$/m, "$1<synced>$2")
    .replace(/^(\s*retainRatio:\s*)\S+(\s*(?:#.*)?)$/m, "$1<synced>$2");
}

/**
 * 已安装副本与包内副本的一致性自检（只读、只警告，绝不改写用户文件）。
 * 自动同步（ensurePresetInstalled）只在**包内预设内容变化**时覆盖；两次升级之间用户对
 * 已安装副本的本地改动会被保留，这里把它指出来（压缩比例由设置页写入，不算差异）。 */
function warnIfPresetDiffersFromBundle(ctx, compositionPath, sourcePath, presetDir) {
  try {
    if (!existsSync(sourcePath)) return;
    if (stripCompactionRatios(readFileSync(compositionPath, "utf8")) === stripCompactionRatios(readFileSync(sourcePath, "utf8"))) return;
    ctx.logger.warn(
      `[dsh-code-pipeline] installed preset at ${presetDir} differs from the bundled copy (${sourcePath}) beyond the settings-written compaction ratios — likely local edits. Automatic sync WILL replace this file on the next bundled-preset change (plugin update); to discard the local edits now, re-copy the bundled preset over it (see README 「升级同步」).`,
    );
  } catch {
    /* 读失败不影响启动 */
  }
...
const PRESET_SYNC_FILE = ".dsh-bundle.json";
...
export function presetSyncPath(presetDir) {
  return join(presetDir, PRESET_SYNC_FILE);
}

/**
 * 包内预设的指纹（版本 + 逐字节内容哈希）。已安装副本记录的是「上次同步时包内预设的哈希」：
 * 一致 → 不动用户文件（两次升级之间的本地改动得以保留）；不一致 → 插件升级了，覆盖它。
 */
export async function presetBundleDigest(sourceDir) {
  const names = [];
  ...
  for (const entryName of names) {
    const content = readFileSync(join(sourceDir, entryName));
    hash.update(entryName);
    hash.update(" ");
  ...
  return { version: packageVersion(), hash: hash.digest("hex") };
}

function readPresetSyncRecord(presetDir) {
  try {
    const record = JSON.parse(readFileSync(presetSyncPath(presetDir), "utf8"));
    return record && typeof record.hash === "string" ? record : undefined;
  } catch {
    return undefined;
  }
}

async function writePresetSyncRecord(presetDir, digest) {
  await writeFile(presetSyncPath(presetDir), JSON.stringify({ version: digest.version, hash: digest.hash, syncedAt: new Date().toISOString() }, null, 2) + "\n", "utf8");
}

/** 逐文件覆盖拷贝；避免整目录 cp 在目标已存在时的嵌套语义。记录文件不在包内，故天然保留。 */
async function copyPresetInto(sourceDir, presetDir) {
  await mkdir(presetDir, { recursive: true });
  for (const entry of await readdir(sourceDir, { withFileTypes: true })) {
    if (!entry.isFile()) continue;
    await cp(join(sourceDir, entry.name), join(presetDir, entry.name), { force: true });
  }
}

/** 覆盖前留一份可恢复副本；放在 temp 下但**不复用** TEMP_ROOT，免得被 24h 清理扫掉。 */
async function backupInstalledPreset(presetDir, preset, version) {
  try {
    const backup = join(tmpdir(), "dsh-code-pipeline-preset-backup", preset + "-" + version + "-" + Date.now());
    await mkdir(dirname(backup), { recursive: true });
    await cp(presetDir, backup, { recursive: true, force: true });
    return backup;
  } catch {
    return undefined;
  }
...
async function ensurePresetInstalled(ctx, dshHome, preset) {
  ...
  try {
    ...
    const digest = await presetBundleDigest(sourceDir);
    const compositionPath = join(presetDir, "agent.cordis.yml");
    const record = readPresetSyncRecord(presetDir);

    // 记录哈希一致 = 已安装副本已经同步过这一版包内预设：绝不覆盖，用户文件原样保留。
    if (existsSync(compositionPath) && record !== undefined && record.hash === digest.hash) {
      ctx.logger.info(`[dsh-code-pipeline] agent preset "${preset}" at ${presetDir} is in sync with the bundled copy (${digest.version})`);
      warnIfPresetPersonaIsStale(ctx, compositionPath, presetDir);
      warnIfPresetDiffersFromBundle(ctx, compositionPath, join(sourceDir, "agent.cordis.yml"), presetDir);
      return "present";
    }

    // 包内预设变了（插件升级）或还没有记录：自动覆盖已安装副本，覆盖前留一份 temp 备份。
    if (existsSync(compositionPath)) {
      const backup = await backupInstalledPreset(presetDir, preset, digest.version);
      await copyPresetInto(sourceDir, presetDir);
      await writePresetSyncRecord(presetDir, digest);
      ctx.logger.info(
        `[dsh-code-pipeline] agent preset "${preset}" refreshed to the bundled copy ${digest.version} at ${presetDir}${record === undefined ? " (no previous sync record — first automatic refresh)" : ""}${backup === undefined ? "" : "; previous copy backed up to " + backup}`,
      );
      warnIfPresetPersonaIsStale(ctx, compositionPath, presetDir);
      return "updated";
    }
    ...
    await writePresetSyncRecord(presetDir, digest);
    ctx.logger.info(`[dsh-code-pipeline] agent preset "${preset}" auto-installed to ${presetDir} (${digest.version})`);
    return "installed";
  } catch (error) {
    ctx.logger.warn(
  ...
  const walk = (rows) => {
    ...
    return rows.map((row) => {
      ...
      if (Array.isArray(row.config)) {
        ...
        if (children !== row.config) return { ...row, config: children };
      }
      return row;
    });
  };
...
/** 按宿主的 settings 能力分派压缩阈值对账：0.1.7 走 configEditor，旧宿主走已安装组合文件。 */
async function reconcileCompaction(ctx, dshHome, preset, thresholdRatio, usesVolatileConfig) {
  if (usesVolatileConfig) return reconcilePresetCompactionViaEditor(ctx, preset, compactionRatios(thresholdRatio));
  return reconcileInstalledCompaction(ctx, dshHome, preset, thresholdRatio);
}

/** 把旧 settings.yaml 的顶层节名改成新条目 id，让宿主的一次性导入直接采纳（仅 dsh 0.1.7）。 */
async function renameLegacySettingsSection(ctx, dshHome, from, to) {
  try {
    const path = join(dshHome, "settings.yaml");
    if (!existsSync(path)) return;
    const text = readFileSync(path, "utf8");
    const renamed = text.replace(new RegExp("^" + from + ":", "m"), to + ":");
    if (renamed === text) return;
    await writeFile(path, renamed, "utf8");
    ctx.logger.info(`[dsh-code-pipeline] renamed legacy settings.yaml section "${from}" -> "${to}" so the Host settings import adopts it`);
  } catch (error) {
    ctx.logger.warn(`[dsh-code-pipeline] could not rename the legacy settings.yaml section: ${error?.message ?? String(error)}`);
  }
}

/**
 * 兜底：宿主可能已把 settings.yaml 改名成 settings.yaml.imported（旧节名未被采纳）。
 * 目标条目还没有用户覆盖时，用 yaml 解析并把旧节 update 进去；yaml 不可解析时跳过。
 */
async function importImportedSettingsSection(settingsCtx, entryId, from) {
  try {
    const dshHome = (typeof process !== "undefined" && process.env && process.env.DSH_HOME) || join(homedir(), ".dsh");
    const path = join(dshHome, "settings.yaml.imported");
    if (!existsSync(path)) return;
    const settings = settingsCtx.settings;
    if (settings === undefined || typeof settings.describe !== "function" || typeof settings.update !== "function") return;
    const descriptor = settings.describe({ redactSecrets: false }).find((row) => row.ns === entryId);
    if (descriptor === undefined) return;
    if (descriptor.user !== undefined && descriptor.user !== null && Object.keys(descriptor.user).length > 0) return;
    const { parse } = await import("yaml");
    const section = parse(readFileSync(path, "utf8"))?.[from];
    if (section === null || typeof section !== "object" || Array.isArray(section)) return;
    await settings.update(entryId, section);
    settingsCtx.logger?.info(`[dsh-code-pipeline] imported legacy settings.yaml.imported section "${from}" into "${entryId}"`);
  } catch (error) {
    settingsCtx.logger?.warn(`[dsh-code-pipeline] legacy settings import skipped: ${error?.message ?? String(error)}`);
  }
}
...
const READ_WIDEN_SCHEMA = Schema.number()
  ...
  .description("read 读取窗口下限（行）：本预设代理（主会话与阶段子代理）发出的 read，请求的 limit 低于该值时会在执行前被就地拓宽到该值——消除「一个文件几十行几十行地翻、每多一步重发整个上下文」的分页模式（嵌套 read 的结果只进 run_code 程序、不进模型历史，拓宽本身零 token 成本，一个程序即可拿足上下文）。超过工具上限 2000 的 limit 会被治愈为 2000，不再整批失败。默认 200；2000 = 一律整窗（等价于省略 limit）；0 = 关闭拓宽。改动立即对后续 read 生效，无需重启（下限数值会随下一次阶段派发注入子代理提示）。");

// 压缩触发比例：@deepseek-ai/dsh-compaction-basic 没有 settings 命名空间，所以这一项
// 由本插件在设置解析出用户值之后 / 设置变更时写进预设声明（dsh 0.1.7 起经 configEditor
// 写 profile patch 里的 preset-<preset> 行；0.1.6 回退写已安装的 .agent-presets 组合）。
const COMPACTION_THRESHOLD_SCHEMA = Schema.number()
  ...
  .description("上下文达到该比例（相对模型窗口）时自动压缩历史，保留量自动取阈值的 1/5。默认 0.5 ≈ 50 万 token（实测 446 个会话中仅 18 个会触发）；调低会更省上下文重发成本，但更快丢掉细节记忆并放弃前缀缓存。改动写入预设声明，对新派发的阶段子代理和新会话生效（当前主会话需重选一次预设或重启 dsh）。");

// schemastery 3.18.3（dsh 0.1.7）才有 .volatile()；旧宿主（≤0.1.6，schemastery 3.18.2）
// 没有该方法。用下面的适配器：旧宿主上字段保持普通配置（走 installSection 回退），
// 新宿主上才是可编辑的 volatile 引用。
...
// （不再是 settings.yaml 命名空间）。业务侧统一经 config.<field>.get() 读取（见 apply）。
...
// dsh 0.1.6 及更早：settings.installSection 需要一个普通（非 volatile）schema。0.1.7 删除
// 了 installSection；这条回退只在旧宿主上生效（见 apply 的宿主能力探测）。
const SettingsSchema = Schema.object({
  stages: STAGES_SCHEMA,
  followupMode: FOLLOWUP_MODE_SCHEMA,
  compactionThresholdRatio: COMPACTION_THRESHOLD_SCHEMA,
  readWidenMinLines: READ_WIDEN_SCHEMA,
});
...
function hostActivationLimit(ctx) {
  ...
  try {
    ...
    // dsh ≤0.1.6：settings.get(namespace) 直接给出解析值。
    if (typeof settings?.get === "function") {
      const value = valid(settings.get("subagent")?.maxActiveSubagents);
      if (value !== undefined) return value;
    }
    // dsh 0.1.7：settings 是 SettingsForms，从 describe() 描述符里取 subagent 条目的值。
...
export async function apply(ctx, config = {}) {
  ...
  // dsh 0.1.7 起预设随包声明为 @deepseek-ai/dsh-agent-preset 行
  // （preset/code-pipeline/cordis.patch.yml），$DSH_HOME/.agent-presets 不再被任何宿主
  // 读取；只有旧宿主（无 volatile 配置）还需要自动安装。
  const dshHome = (typeof process !== "undefined" && process.env && process.env.DSH_HOME) || join(homedir(), ".dsh");
  const hasVolatileRef = (value) => value !== null && typeof value === "object" && typeof value.get === "function";
  const usesVolatileConfig = hasVolatileRef(config.stages) || hasVolatileRef(config.followupMode)
    || hasVolatileRef(config.compactionThresholdRatio) || hasVolatileRef(config.readWidenMinLines);
  if (!usesVolatileConfig) await ensurePresetInstalled(ctx, dshHome, preset);
  await cleanOldTempFiles();

  // 设置来源。
  //   dsh 0.1.7：Config 的可编辑字段是 .volatile() 引用，每次调用读 config.<field>.get()
  //     即得最新值（settings 服务把表单写入 profile patch，loader 热更新引用）。
  //   dsh ≤0.1.6：没有 volatile 引用，回退 ctx.settings.installSection（普通 settings 节）。
  ...
  // 压缩触发比例 → 预设声明（dsh 0.1.7 经 configEditor 写 profile patch 的
  // preset-<preset> 行；旧宿主写已安装的 .agent-presets 组合）。写入只对「之后挂载」
  // 的代理生效，所以每次设置变化都重新对账（幂等：值没变不写盘）。
  const syncCompactionThreshold = () => {
    void reconcileCompaction(ctx, dshHome, preset, source()?.compactionThresholdRatio, usesVolatileConfig);
  };

  if (usesVolatileConfig) {
    // 旧 settings.yaml 的 code-pipeline 节：在宿主的一次性导入之前改成新条目 id，
    // 让宿主直接采纳（避免旧数据留在 settings.yaml.imported 里）。
    await renameLegacySettingsSection(ctx, dshHome, "code-pipeline", "dsh-code-pipeline");
  }

  ctx.inject(["settings"], (settingsCtx) => {
    const settingsService = settingsCtx.settings;
    if (usesVolatileConfig) {
      // 兜底：宿主已把 settings.yaml 改名成 .imported 时，目标条目还没有用户覆盖则直接导入旧节。
      void importImportedSettingsSection(settingsCtx, "dsh-code-pipeline", "code-pipeline")
        .then(() => syncCompactionThreshold());
      // volatile 改动即时反映到工具调用（source 每次读引用）；压缩阈值另需防抖写回预设声明。
      // 防抖也避开与 settings 写入（configEditor.edit -> loader 热更新）的重入。
      let scheduled = false;
      const schedule = () => {
        if (scheduled) return;
        scheduled = true;
      ...
      settingsCtx.effect(() => settingsCtx.on("loader/volatile-update", () => schedule()), "code-pipeline: compaction reconcile");
      return;
    }
    // dsh ≤0.1.6 回退：installSection（0.1.7 已删除，先做能力探测避免抛错）。
    if (settingsService === undefined || typeof settingsService.installSection !== "function") return;
    settingsService.installSection(ctx, "code-pipeline", SettingsSchema, config, {
      setSource: (next) => {
        source = next;
        syncCompactionThreshold();
      },
      onChange: () => {
        // 无需重新注入：工具调用时读取最新设置；压缩触发比例另需写进预设声明。
        syncCompactionThreshold();
      },
    });
  });
  ...
  const isPipelineReader = (agent) => {
    ...
    try {
      return ctx.get("agentPresets")?.composedPreset(agent.ctx) === preset;
    } catch {
      return false;
    }
  };
