...
async function materializeDispatchMessage(stage, args, promptText, cfg) {
  ...
  for (const input of STAGE_INPUTS[stage.key]) {
    ...
    if (input.key === "files") {
      // 显式「没有候选清单」：单个 -（或 none / n/a）。允许声明没有，不允许静默省略。
...
export async function apply(ctx, config = {}) {
  ...
  function drainPendingReadNotices(agent) {
    ...
    for (const list of bucket.values()) {
      for (const path of list) if (!paths.includes(path)) paths.push(path);
    }
    bucket.clear();
    if (paths.length === 0) return undefined;
    return readNotice(paths, undefined);
  }
