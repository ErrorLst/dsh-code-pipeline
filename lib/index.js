  ...
  return {
    ...
    output: {
      ...
      render: (_args, value) => [
        {
          type: "text",
          text: value.kind === "foreground" ? outputValueText(value.output) : String((value && value.runId) || ""),
        },
      ],
    },
    ...
    async execute(args, exec) {
      ...
      // 子代理启动失败（provider/凭据/启动错误）也视为阶段不可用：
      // 把原始错误转为 unavailable 错误,携带"停止并告知用户"指令。
      let run;
      try {
...
export function apply(ctx, config = {}) {
  ...
  // 预设安装提示：预设文件随项目在 preset/ 目录维护，但插件不会安装或更新它——
  // 需要手动拷贝到 $DSH_HOME/.agent-presets 下（见 README「预设文件」）。
  ...
  if (!existsSync(presetDir)) {
    ctx.logger.warn(
      `[dsh-code-pipeline] agent preset "${preset}" is NOT installed at ${presetDir}; copy it from the plugin's preset/ directory (see README) — otherwise no session composes the pipeline persona`,
    );
  }
