...
const STAGES = [
  ...
  },
  {
    ...
    description:
      "Pipeline IMPLEMENTATION stage: delegate the exact implementation of a plan to one full-tool stage subagent (it may modify the workspace). The subagent returns a summary of changes. The stage model is configured in Settings → 代码流水线.",
  },
  {
    key: "review",
    ...
    description:
      "Pipeline REVIEW stage: delegate a read-only audit of a change set to one stage subagent. REVIEWING ONLY — never use it to plan, design, or propose implementations (that is subagent_plan's job). Its reply starts with APPROVED or CHANGES REQUIRED: followed by numbered issues. The stage model is configured in Settings → 代码流水线.",
  },
];
  ...
  return {
    ...
    parameters: {
      description: {
        type: "string",
        required: true,
        description: "A short (3-5 word) description of the delegated stage task, for display.",
      },
      prompt: {
        type: "string",
        required: true,
      ...
      run_in_background: {
        type: "boolean",
        description:
          "Default false: wait for the stage result in the foreground. Set true to start a durable background run and return its subagentId immediately — use this when the stage may outlast the current turn (no tool-level timeout applies to the foreground wait, but a long run can exceed the turn's lifetime). The runtime notifies this session when it settles; track status with list_agents and fetch the result with subagent.history.",
      },
    },
    output: {
      schema: {
