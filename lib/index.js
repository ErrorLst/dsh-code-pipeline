...
const STAGE_INPUTS = {
  plan: [
    { key: "files", hint: "NEWLINE-SEPARATED list of every path the orchestrator already opened or shortlisted as relevant. The planner reads ALL of them in ONE program before it starts — pass the real list here, not only inside `context`. The child reads whole files by OMITTING `limit` (the read tool caps it at 2000 lines and rejects larger values)." },
  ...
  impl: [
    { key: "files", hint: "NEWLINE-SEPARATED list of the files this workstream owns (the plan's `owned files`). The implementer reads ALL of them in ONE program before its first edit, OMITTING `limit` (the read tool caps it at 2000 lines and rejects larger values)." },
  ...
  review: [
    { key: "files", hint: "NEWLINE-SEPARATED list of the changed files (plus any file the reviewer must open). The reviewer reads them in ONE program before auditing, OMITTING `limit` (the read tool caps it at 2000 lines and rejects larger values)." },
...
function renderFilesBlock(value) {
  ...
  return "**files** — "
    ...
    + (declaredNone ? "" : "\n" + value)
    + " Read whole files by OMITTING `limit` — `tools.read` caps it at 2000 lines and REJECTS anything larger, so one `limit: 2500` fails EVERY call in the batch. Build the argument object by OMITTING optional keys too: a key whose value is `undefined` (`{ include: undefined }`) is rejected before dispatch as non-lossless JSON.";
}
