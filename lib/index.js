...
async function materializeDispatchMessage(stage, args, promptText, cfg) {
  ...
  for (const input of STAGE_INPUTS[stage.key]) {
    ...
    if (input.key === "files") {
      parts.push(
        "\n\n**files** — the orchestrator's already-identified candidate set. BEFORE you plan, audit or edit anything, "
        + "read EVERY path below in ONE run_code program (parallel `tools.read` calls, whole files or the widest range that matters). "
