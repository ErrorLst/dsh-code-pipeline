...
const READ_ONLY_TOOLS = [
  ...
  "web_search",
  "ask_user_question",
  // 插件给阶段子代理注入的回执通道（结构化 I/O）：它只把 envelope 交给插件，
  // 不能读写工作区，因此仍然符合"只读阶段"的定位。注册发生在 agent/created，
  // 与 tools.restrict 的先后顺序由单测锁住（restrict 是白名单，名字必须在名单里）。
  "pipeline_submit",
];
...
// 阶段子代理必须把结论作为**结构化对象**交回来，而不是一段自由文本：
//   1. 能调用 pipeline_submit 的阶段子代理（插件给它们注入了该工具）在**调用点**
//      接受 schema + 语义校验——不合格当场打回，子代理可以立刻改，不必花掉一整轮；
//   2. 兜底通道：插件在 subagent/end 里从子代理最终回复的最后一个 ```json 围栏解析。
// 两条通道写进同一条台账（dispatched.get(childId).result），由 pipeline_result 读出。
//
...
async function materializeDispatchMessage(stage, args, promptText, cfg) {
  ...
  // 结构化回执契约随派发一起下发：子代理因此不用猜要交什么形状；能用 pipeline_submit
  // 的阶段子代理在**调用点**就拿到 schema 校验，用不了的仍可把同样的对象放进 json 围栏。
  parts.push(
    "\n\n**requiredOutput**\nEnd your reply with ONE json-fenced envelope matching this schema"
    + " (and, when the pipeline_submit tool is available to you, submit the same object with it):\n"
...
/**
 * 语义校验——JSON Schema（由宿主在 pipeline_submit 的调用点执行）表达不了的规则。
...
/**
 * 阶段子代理的回执工具（注入到**子代理**作用域）。schema 由宿主在调用点校验，
 * 语义规则由 validateEnvelope 校验；不合格直接抛错打回，子代理当轮即可修正。
 */
function buildSubmitToolDef(stage) {
  const schema = ENVELOPE_SCHEMAS[stage.key];
  const label = stage.key === "impl" ? "implementation" : stage.key;
  return {
    name: "pipeline_submit",
    description:
      `Submit your structured ${label} envelope to the pipeline plugin. Call this ONCE, at the END of your turn, with the SAME object you put in your json fence. `
      + "The envelope is validated AT THIS CALL, so a malformed one is rejected here and you can fix it immediately instead of costing the orchestrator a whole round. "
      + "Your human-readable Markdown reply is still required — this is only the machine-readable half. "
      + `Envelope schema: ${JSON.stringify(schema)}`,
    parameters: {
      type: "object",
      additionalProperties: false,
      required: ["envelope"],
    ...
    output: {
      schema: {
        ...
        additionalProperties: false,
        required: ["ok", "kind"],
        properties: { ok: { type: "boolean", const: true }, kind: { type: "string" } },
      },
      render: (_args, value) => [{ type: "text", text: `accepted the ${String(value.kind)} envelope` }],
    },
    isConcurrencySafe: () => false,
    async execute(args, exec) {
      const childId = String(exec?.agent?.id ?? "");
      const entry = dispatched.get(childId);
      if (entry === undefined) throw new Error("pipeline_submit: this agent is not a tracked stage child of this plugin");
      const value = args?.envelope;
      const problems = validateEnvelope(stage.key, value);
      if (problems.length > 0) {
        throw new Error(`pipeline_submit: the ${stage.key} envelope was rejected — ${problems.join("; ")}. Fix these and call pipeline_submit again.`);
      }
      entry.result = { parsed: true, source: "submit", value };
      if (stage.key === "review") recordReviewOutcome(entry, value);
      return { ok: true, kind: String(value.kind ?? stage.key) };
    },
  };
}
...
function buildResultToolDef(ctx) {
  return {
    ...
    description:
      "Read the STRUCTURED envelope a stage subagent produced (submitted via pipeline_submit, or parsed from the final json fence as a fallback). "
...
export async function apply(ctx, config = {}) {
  ...
  // 阶段子代理的回执工具（结构化 I/O）。宿主把 startContinuable 的 agentOptions 原样展开到
  // agent.options（child-agent.ts），所以 stageKey 就是识别阶段子代理的判据——与创建上限、
  // 阶段归属用的是同一个字段。注意：dispatched 台账此刻**还没**写入（派发方在
  // startContinuable 返回之后才 set），因此不能靠台账识别，否则会漏注入。
  // 只读阶段的工具面是白名单（READ_ONLY_TOOLS），pipeline_submit 必须在名单里才可见。
  const injectedSubmit = new WeakSet();
  const injectSubmit = (agent) => {
    if (agent === null || agent === undefined || injectedSubmit.has(agent)) return;
    const stageKey = agent?.options?.stageKey;
    if (typeof stageKey !== "string") return;
    const stage = STAGES.find((candidate) => candidate.key === stageKey);
    if (stage === undefined) return;
    const tools = agent?.ctx?.get?.("tools");
    if (tools === undefined || typeof tools.register !== "function") return;
    tools.register(buildSubmitToolDef(stage));
    injectedSubmit.add(agent);
  };
  ctx.on(
    "agent/created",
    ({ agent }) => {
  ...
  // 清空也解决了另一个歧义——pipeline_submit 在激活期间写入 result，end 只补空缺。
  ...
  ctx.on("subagent/end", (info) => {
    ...
    const entry = childId === "" ? undefined : dispatched.get(childId);
    if (!entry) return;
    // 结构化回执的兜底通道：本次激活没有调用 pipeline_submit 时，从最终回复的 json
    // 围栏解析。解析失败**不阻塞**——只记录原因，pipeline_result 会如实报出来。
    if (entry.result === undefined) {
      const parsed = extractEnvelope(info?.lastAssistantMessage);
      entry.result = parsed.value === undefined
        ? { parsed: false, reason: parsed.reason }
        : { parsed: true, source: "parsed", value: parsed.value };
      if (parsed.value !== undefined && entry.stage === "review") recordReviewOutcome(entry, parsed.value);
    }
