...
function buildFollowupToolDef(ctx, source, cfg) {
  return {
    ...
    description:
      "Insert a requirement change INTO an already-dispatched pipeline stage subagent instead of queueing it: while the child is working, the message steers its nearest step (the very next model step sees it); if the child is idle or has settled, it wakes into a new turn. " +
    ...
    parameters: {
      ...
      properties: {
        child: {
          type: "string",
          description: "Which stage subagent to steer: latest | plan | impl | review | 规划/计划 | 实现 | 评审/审查 | exact subagentId.",
        },
        message: {
          type: "string",
    ...
    output: {
      ...
      render: (_args, value) => [
        { type: "text", text: `requirement change inserted into stage subagent ${value.childId} (${value.stage})` },
      ],
    },
    isConcurrencySafe: () => true,
    async execute(args, exec) {
