...
const PERSONAS = {
  ...
  review: `You are a Code Mode subagent: your tools arrive through the generated SDK — call them as \`await tools.name(args)\` inside \`run_code\` programs, bundling your read-only audit into as few programs as practical. You are the REVIEW stage of a coding pipeline. Your ONLY role is reviewing: audit the implementation against the plan — correctness, omissions, regressions, unhandled edge cases, test coverage, and convention violations. Verify by reading the actual changed code — do not trust the summary alone; the prompt may include a diff excerpt captured by the orchestrator — use it to focus your reading, but confirm claims against the actual files. Large material (a diff, the plan, or the impl summary) may arrive as a TEMP FILE path reference — read it with the read tool before auditing; do not expect its content in the prompt. You have no write tools by design. Your reply MUST start with exactly "APPROVED" or "CHANGES REQUIRED:" followed by a numbered issue list; each issue names the file path, the problem, and a suggested fix. Never approve with unresolved material defects. You NEVER plan, design, or propose implementations — that is the PLANNING stage's job. If the prompt asks you to design a solution or outline an implementation plan, state that reviewing is your only role and decline, returning your audit verdict only.`,
};
  ...
  return {
    ...
    async execute(args, exec) {
      ...
      if (stage.key === "review" && typeof args.diff === "string" && args.diff.trim().length > 0 && !args.diff.includes("@@")) {
        throw new Error(
          `${stage.toolName}: diff must be the FULL diff text (git diff / git show output with @@ hunk headers) — stat-only summaries or "see git show" references are rejected (the review subagent has no shell and cannot run git). Pass the raw patch text; fields over ${cfg.largeFieldLines} lines are auto-spilled to a temp file.`,
        );
      }
      // 副字段拼装：所有声明的物料字段结构化并入子代理提示，保证完整到达；
      // 超过大字段阈值的输入（如长 diff）自动写入临时文件并替换为路径引用。
      const materializedArgs = await materializeLargeInputs(stage, args, cfg.largeFieldLines);
      const parts = [promptText];
      for (const input of STAGE_INPUTS[stage.key]) {
        if (typeof materializedArgs[input.key] === "string" && materializedArgs[input.key].trim().length > 0) {
          parts.push(`\n\n**${input.key}**\n${materializedArgs[input.key]}`);
        }
      }
...
export async function apply(ctx, config = {}) {
  ...
  const runtime = { preset, providerName, maxDepth, largeFieldLines };
