...
//
// 为什么默认下限 = 2000（= 工具上限，即整窗）而不心疼：本预设所有代理都是 PTC
// （Code Mode），read 只在 run_code 程序内部发生——嵌套工具结果**只进程序、不进
// 模型历史**，拓宽本身零 token 成本，却让一个程序一次拿足上下文；模型只为它
// PRINT 的蒸馏结果付费。结果自带行号与 totalLines，窗口被拓宽是自描述的。
// 下限可经设置项 readWidenMinLines 调低（如 500）或置 0 关闭。

/** read 工具的行数上限（缺省 limit 也是它）：与宿主 dsh-tool-fs 的 caps.limit 一致。 */
export const READ_TOOL_MAX_LINES = 2000;

/** readWidenMinLines 的出厂默认：2000 = 一律整窗（等价于省略 limit）。 */
export const READ_WIDEN_DEFAULT_LINES = READ_TOOL_MAX_LINES;
...
const READ_WIDEN_SCHEMA = Schema.number()
  ...
  .description("read 读取窗口下限（行）：本预设代理（主会话与阶段子代理）发出的 read，请求的 limit 低于该值时会在执行前被就地拓宽到该值——消除「一个文件几十行几十行地翻、每多一步重发整个上下文」的分页模式（嵌套 read 的结果只进 run_code 程序、不进模型历史，拓宽本身零 token 成本，一个程序即可拿足上下文）。超过工具上限 2000 的 limit 会被治愈为 2000，不再整批失败。默认 2000 = 一律整窗（等价于省略 limit）；0 = 关闭拓宽。改动立即对后续 read 生效，无需重启。");
