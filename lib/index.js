...
//   - 每个阶段工具在每次 CALL 时读取当前设置（stages / followupMode /
//     readWidenMinLines），因此设置页修改立即生效，无需重启 dsh，也无需重新注入；
//   - 设置持久化：插件 Config 的 volatile 字段（值持久化到 profile 的 Cordis patch，
//     客户端经 configForms 读写）；工具每次调用读引用，所以保存立即生效。
//
...
//
// 部署兼容性：
//   1. 导出的 Config 提供全部字段默认值，apply 内仍对组合字段做 ?? 兜底（防止旧宿主
//      不解析 Config 时注入守卫把 "code-pipeline" 与 undefined 比较后静默跳过注入）；
//   2. 宿主 context 对未声明 inject 的名称抛 "cannot get property ... without
//      inject"，因此所有服务访问统一走 lib/host.js 的 getService(ctx, id)（声明式 inject 保持最小；
...
import { HOST_CONTRACT } from "./host-contract.js";
import { describeHostChecks, getService, onHost, probeHost } from "./host.js";
...
// ── read 读取窗口拓宽（read widening）：tools/execute 执行前就地把小 limit 拓宽 ──
//
// 解决的浪费模式：模型（尤其阶段子代理）常对一个文件「几十行几十行」地翻——一个
// run_code 程序读 50 行，思考一步，再开一个程序读下 50 行。每多一步都要把整段
// 上下文重发一遍（实测 impl 子代理单轮 62 步、97% 的 token 是 cacheRead），而多读
// 的那点内容只随上下文重发一次：步数才是大头。persona（CONTEXT ECONOMICS）与
// 读卫生提醒只能「劝」；这里在宿主的 tools/execute around-waterfall 上做**机械
// 保证**——流水线代理发出的 read，请求的 limit 低于下限就在执行前拓宽到下限，
// 超过上限（2000，read 工具会直接报错）则治愈为上限（0.3.7 实测一次 limit: 2500
// 让 25 个 read 全部失败、下一步全部重读）。
//
// 默认下限 200（0.4.5 曾为 2000、0.4.6 曾为 500，实测反馈后继续调低）。机械拓宽
// 只保证「单次读够大」；模型若不知道有下限，仍会主动写出一串 10~60 行的小窗读
// （同一个 run_code 程序里对同一文件连读多次、窗口互相重叠）。所以机制之外还
// **告知**：预设 persona 的 Step economy 写明「读有下限、别把文件切成小窗」；
// 三段阶段 persona 的 TOOL GOTCHAS 同步；files 派发块注入**实时**下限值
// （renderFilesBlock 第二参数，设置页改完下一次派发即生效）。想调大下限改设置
// 即可（2000 = 工具上限 = 一律整窗），置 0 关闭。已保存过旧值的安装显式存储
// 优先于默认，需在设置页重存一次。结果自带行号与 totalLines，窗口自描述。

/** read 工具的行数上限（缺省 limit 也是它）：与宿主 dsh-tool-fs 的 caps.limit 一致。 */
export const READ_TOOL_MAX_LINES = 2000;

/** readWidenMinLines 的出厂默认（0.4.7 起 200）：连「10~60 行小窗试探」也一并拦下；2000 = 一律整窗。 */
export const READ_WIDEN_DEFAULT_LINES = 200;

/**
 * 设置值 → 拓宽下限：0 = 关闭拓宽；合法区间 [1, READ_TOOL_MAX_LINES]；
 * 非法 / 缺省回落出厂默认。纯函数，测试直接 import。
 */
export function normalizeReadWidenFloor(value) {
  const numeric = typeof value === "number" ? value : Number.parseFloat(String(value ?? READ_WIDEN_DEFAULT_LINES));
  if (!Number.isFinite(numeric)) return READ_WIDEN_DEFAULT_LINES;
  if (numeric <= 0) return 0;
  return Math.min(READ_TOOL_MAX_LINES, Math.max(1, Math.round(numeric)));
}

/**
 * 计算一次 read 调用的拓宽后参数（纯函数，测试直接 import）。
 * 规则（args 是 read 的原始参数对象）：
 *   - 不是 read 形状（缺 file_path，或 limit 缺省/非正整数）→ undefined：
 *     limit 缺省时宿主默认就是上限整窗，无需动；
 *   - limit > 上限 → 治愈为上限（healed: true）——不治就整批失败、下一步全部重读；
 *   - limit < 下限（floor 归一后 > 0）→ 拓宽到下限（healed: false）；
 *   - 下限 ≤ limit ≤ 上限 → undefined（尊重模型的显式选择）。
 * 返回 { arguments, from, to, healed }；arguments 是新对象，其余键原样保留。
 */
export function widenReadArguments(args, floor, maxLines = READ_TOOL_MAX_LINES) {
  if (args === null || typeof args !== "object") return undefined;
  if (typeof args.file_path !== "string" || args.file_path.length === 0) return undefined;
  if (!Number.isInteger(args.limit) || args.limit <= 0) return undefined;
  if (!Number.isInteger(maxLines) || maxLines <= 0) return undefined;
  if (args.limit > maxLines) {
    return { arguments: { ...args, limit: maxLines }, from: args.limit, to: maxLines, healed: true };
  }
  const target = Math.min(floor, maxLines);
  if (!Number.isInteger(target) || target <= 0 || args.limit >= target) return undefined;
  return { arguments: { ...args, limit: target }, from: args.limit, to: target, healed: false };
}
...
const STAGE_INPUTS = {
  plan: [
    { key: "files", hint: "NEWLINE-SEPARATED list of every path the orchestrator already opened or shortlisted as relevant. The planner reads ALL of them in ONE program before it starts — pass the real list here, not only inside `context`. The child reads whole files by OMITTING `limit` (the read tool caps `limit` at 2000 lines; small limits are auto-widened and over-cap ones healed)." },
  ...
  impl: [
    { key: "files", hint: "NEWLINE-SEPARATED list of the files this workstream owns (the plan's `owned files`). The implementer reads ALL of them in ONE program before its first edit, OMITTING `limit` (the read tool caps `limit` at 2000 lines; small limits are auto-widened and over-cap ones healed)." },
  ...
  review: [
    { key: "files", hint: "NEWLINE-SEPARATED list of the changed files (plus any file the reviewer must open). The reviewer reads them in ONE program before auditing, OMITTING `limit` (the read tool caps `limit` at 2000 lines; small limits are auto-widened and over-cap ones healed)." },
...
"Pipeline PLAN stage. CONTRACT: params = prompt|task (task text; one of them required) + description (display label, optional) + context? + files (REQUIRED — newline-separated paths the planner must read; "-" declares none) + constraints? (all merged into the child's prompt, never dropped). Unknown keys are rejected. BACKGROUND-ONLY: returns { kind: 'continuable', subagentId } immediately; the runtime notifies this session when it settles — the child's final reply arrives in this session as a settled notice; read the plan from that notice. PLANNING ONLY: never use it to review/audit/verify/approve code (that is subagent_review's job). PRESENTATION RULE: print the plan as a normal Markdown reply and END the turn — NEVER call ask_user_question to confirm it. The stage model is configured in Settings → 代码流水线. The whole dispatch message (prompt + all stage inputs) is written to a SINGLE temp file automatically — the child reads that one file with the read tool; never inline large material into these fields (inline generation is truncated at the model output limit)." +
      ...
      " CONCURRENCY: each stage has its own RUNNING cap (Settings → 代码流水线 → 最大并发; 0 = unlimited) — how many children of that stage may run at ONCE for this session. There is NO cap on how many children may be CREATED in total: a NEW workstream always gets its OWN fresh child, regardless of how many children came before it. A call rejected for the running cap is TRANSIENT, not a stage failure — do NOT stop the task and do NOT hand a new workstream to an unrelated child: wait for a settled notice (which frees a running slot), then dispatch the remaining independent workstream in a LATER step as its own fresh child. Continuing the SAME workstream's later rounds (review findings, a mid-flight requirement change, a wall-clock continue) still goes to that workstream's existing child with pipeline_followup (always with a files list) — that creates no child. When independent targets are genuinely first-round work, dispatch them in PARALLEL in ONE program (Promise.all); if the running cap admits fewer than you dispatched, re-dispatch the remainder later.",
  },
  {
    key: "impl",
...
/**
 * 渲染「待读清单」硬指令块。阶段派发与 pipeline_followup 复用同一段文案——
 * 0.3.8 的教训：复用路径曾经只发一段自由文本，子代理于是把整份清单重新发现一遍
 * （实测一个复用的 impl 在开头花了 7 步 / 15 次 read 才重新找到 core-settings 的文件）。
 *
 * `-`（或 none / n/a）是显式「没有清单」：允许声明没有，不允许静默省略。
 *
 * @param value - the newline-separated path list, or the "none" sentinel.
 * @param readFloor - 当前的 read 拓宽下限（0 = 关闭）；数值注入文案，让子代理
 *   知道小 limit 会被拓宽、从而不要主动把文件切成小窗读。
 * @returns the model-facing block (no leading newlines).
 */
function renderFilesBlock(value, readFloor) {
  const declaredNone = /^(?:-|none|n\/a|\(none\))$/i.test(value.trim());
  return "**files** — "
    + (declaredNone
      ? "the orchestrator supplied NO candidate list (explicitly declared none), so do your own reconnaissance: locate the files with glob/grep and then read every candidate in ONE run_code program — never one file per step."
      : "the orchestrator's already-identified candidate set. BEFORE you plan, audit or edit anything, read EVERY path below in ONE run_code program (parallel `tools.read` calls, whole files or the widest range that matters). Do not spend steps re-deriving the file list; open additional files only when this set leaves a real gap.")
    + (declaredNone ? "" : "\n" + value)
    + " Read whole files by OMITTING `limit` — `tools.read` caps it at 2000 lines (the plugin heals an over-cap `limit` down to 2000 and auto-widens a small one up to the read floor, so a batch no longer fails on `limit` — but OMIT it anyway: that is the whole-window default)."
    + (Number.isInteger(readFloor) && readFloor > 0
      ? ` Your READ FLOOR is ${readFloor} lines: any limit below it is auto-widened to ${readFloor}, so NEVER chunk one file into successive small reads — a 10-60-line ask simply returns a ${readFloor}-line window that already covers any sub-${readFloor} follow-up. Ask for the whole file, or a range of at least ${readFloor} lines, ONCE per region.`
      : " Read widening is disabled (floor 0): request exactly the range you need.")
    + " Build the argument object by OMITTING optional keys too: a key whose value is `undefined` (`{ include: undefined }`) is rejected before dispatch as non-lossless JSON.";
}
async function materializeDispatchMessage(stage, args, promptText, cfg, readFloor) {
  const parts = [promptText];
  for (const input of STAGE_INPUTS[stage.key]) {
    const value = args[input.key];
    if (typeof value !== "string" || value.trim().length === 0) continue;
    // files 是「待读清单」，不是普通背景字段：给它一段硬指令，让子代理第一步就用**一个**
    // 程序把清单整个读进来，而不是从散文 context 里再解析出文件名、然后一步开一个。
    if (input.key === "files") {
      parts.push("\n\n" + renderFilesBlock(value, readFloor));
      continue;
    }
    parts.push("\n\n**" + input.key + "**\n" + value);
  }
...
// ── 阶段设置 schema（volatile 字段，持久化到 profile 的 Cordis patch）──────────
...
const STAGE_SCHEMA = Schema.object({
  ...
  reasoningEffort: Schema.string()
    .default("")
    .description("思考等级：留空继承 provider 路由级默认；可选值以所选模型实际支持面为准"),
  maxConcurrency: Schema.number()
    .min(0)
    .max(64)
    .default(0)
    .description("同一父会话内该阶段同时在跑的子代理上限；0 = 不限制（仍受宿主每会话同时存活上限与每程序 10 个并行子调用约束）"),
  budgetMinutes: Schema.number()
    .min(0)
    .max(1440)
    .default(0)
    .description("该阶段单次派发的墙钟预算（分钟）；0 = 不限制。到点中断当前回合并自动索取收尾报告，之后仍可用 pipeline_followup 续跑"),
});

// ── 压缩触发比例：设置 → profile 预设声明行的对账 ─────────────────────────────
// @deepseek-ai/dsh-compaction-basic 没有 settings 命名空间（static inject =
// ['llm','tokenMeter','sessions']），它的 thresholdRatio 只能来自挂载它的组合。
// 「设置页可调」的落法因此是：本插件把设置值经宿主 configEditor 写进 profile patch
// 里 "preset-<preset>" 声明行的 compaction-basic 配置（reconcilePresetCompactionViaEditor，
// 幂等：值没变不写盘）。写入只对**之后挂载**的代理生效（新会话、新派发的阶段子代理），
// 无需重启 dsh；已挂载的代理留在它加入的那一代。
//
// 写两个**比例**而不是 token 数：thresholdRatio / retainRatio 都是「路由后模型窗口」
// 的比例，于是这一对值与窗口无关——本插件不需要知道某个阶段被路由到哪个模型、窗口
// 多大（那是按阶段、按模型变化的）。宿主强制「保留必须严格小于阈值」，
// r/5 < r 在整个取值区间自动成立。出厂默认 0.5/0.1 在 100 万窗口上 = 50 万触发、
// 保留 10 万逐字。
const COMPACTION_ROW_ID = "compaction-basic";
/** 触发比例的允许区间与出厂默认，与 COMPACTION_THRESHOLD_SCHEMA 保持一致。 */
...
/**
 * 在预设声明里对账 compaction-basic 的 thresholdRatio / retainRatio。
 * dsh 0.1.7 的预设是 profile patch 里的 "preset-<preset>" 行，行内的 plugins
 * 可能把 compaction-basic 嵌在 group 的 config 数组里，故递归查找。纯函数：
 * 返回新的 plugins 数组与是否发生改动；调用方只在 changed 时写回。
 */
export function reconcileCompactionPlugins(plugins, ratios) {
  let changed = false;
  const walk = (rows) => {
    if (!Array.isArray(rows)) return rows;
    // 只有子树真的变了才重建：`rows.map()` 恒返回新数组，若直接返回它，函数「未变化
    // 时返回原树」的契约就不成立——而调用方会把这个结果写回**宿主 profile patch**。
    let touched = false;
    const next = rows.map((row) => {
      if (row === null || typeof row !== "object" || Array.isArray(row)) return row;
      if (row.id === COMPACTION_ROW_ID) {
        const config = row.config !== null && typeof row.config === "object" && !Array.isArray(row.config) ? { ...row.config } : {};
        if (config.thresholdRatio !== ratios.thresholdRatio || config.retainRatio !== ratios.retainRatio) {
          config.thresholdRatio = ratios.thresholdRatio;
          config.retainRatio = ratios.retainRatio;
          changed = true;
          touched = true;
          return { ...row, config };
        }
      ...
      if (Array.isArray(row.config)) {
        const children = walk(row.config);
        if (children !== row.config) {
          touched = true;
          return { ...row, config: children };
        }
      }
      return row;
    });
  ...
  return { plugins: walk(plugins), changed };
}

/**
 * dsh 0.1.7：把压缩比例写进 profile patch 里的 "preset-<preset>" 声明行（经 configEditor）。
 * 未变化时不写盘——避免每次启动都把整份预设组合固化进用户 profile patch。
 * 任何失败只告警，不影响启动。
 */
async function reconcilePresetCompactionViaEditor(ctx, preset, ratios) {
  try {
    const editor = getService(ctx, "configEditor");
    if (editor === undefined || typeof editor.edit !== "function" || typeof editor.entries !== "function") {
      ctx.logger.warn("[dsh-code-pipeline] configEditor service unavailable; skipping the compaction threshold sync");
      return "unavailable";
    }
    const entryId = "preset-" + preset;
    const entry = editor.entries().find((candidate) => candidate?.options?.id === entryId);
    if (entry === undefined) {
      ctx.logger.warn(
        `[dsh-code-pipeline] no "${entryId}" profile entry — is the bundled preset patch (preset/${preset}/cordis.patch.yml) mounted? skipping the compaction threshold sync`,
      );
      return "not-found";
    }
    const preview = reconcileCompactionPlugins((entry.options.config ?? {}).plugins, ratios);
    if (!preview.changed) return "unchanged";
    await editor.edit(entry, (live) => {
      // 写的是宿主 profile patch：形状可疑时**原样返回**（等于不写），绝不把
      // 结构不确定的树交给编辑器落盘。
      if (live === null || typeof live !== "object" || Array.isArray(live)) return live;
      if (!Array.isArray(live.plugins)) return live;
      const next = reconcileCompactionPlugins(live.plugins, ratios);
      return next.changed ? { ...live, plugins: next.plugins } : live;
    });
    ctx.logger.info(
      `[dsh-code-pipeline] compaction threshold synced into profile entry "${entryId}": thresholdRatio ${ratios.thresholdRatio}, retainRatio ${ratios.retainRatio} — takes effect for agents mounted after this write (new sessions and newly dispatched stage subagents)`,
    );
    return "updated";
  } catch (error) {
    ctx.logger.warn(
      `[dsh-code-pipeline] failed to sync the compaction threshold into the "${preset}" preset declaration: ${error?.message ?? String(error)}; set thresholdRatio / retainRatio in the preset row's compaction-basic config by hand`,
    );
    return "error";
  }
}

/** 把压缩触发比例对账进 profile 的 preset-<preset> 声明行（dsh 0.1.7 经 configEditor）。 */
async function reconcileCompaction(ctx, preset, thresholdRatio) {
  return reconcilePresetCompactionViaEditor(ctx, preset, compactionRatios(thresholdRatio));
}
...
const FOLLOWUP_MODE_SCHEMA = Schema.union([Schema.const("steer"), Schema.const("queue")])
  .default("steer")
  .description("pipeline_followup 的默认投递方式：steer = 插入（运行中下一步即收到）；queue = 排队（当前回合结束后处理）");

/** read 窗口拓宽下限（行）：本预设代理发出的 read 低于该 limit 就地拓宽到该值；0 = 关闭。 */
const READ_WIDEN_SCHEMA = Schema.number()
  .min(0)
  .max(READ_TOOL_MAX_LINES)
  .default(READ_WIDEN_DEFAULT_LINES)
  .description("read 读取窗口下限（行）：低于该值的 read 请求在执行前被就地拓宽到该值，超过 2000 的治愈为 2000。默认 200；2000 = 一律整窗；0 = 关闭。改动立即生效");

// 压缩触发比例：@deepseek-ai/dsh-compaction-basic 没有 settings 命名空间，所以这一项
// 由本插件在设置解析出用户值之后 / 设置变更时经 configEditor 写进 profile patch 的
// preset-<preset> 声明行。
const COMPACTION_THRESHOLD_SCHEMA = Schema.number()
  ...
  .description("上下文达到该比例（相对模型窗口）时自动压缩历史，保留量取阈值的 1/5。写入 profile 的预设声明，对新会话与新派发的阶段子代理生效");

// schemastery 3.18.3（dsh 0.1.7）才有 .volatile()；本插件要求 dsh >= 0.1.7-alpha.1，
// 但保留这个一行适配器：schemastery 缺失/变旧时字段退化为普通配置，而不是让
// Config 求值抛错（那会连带打断插件挂载）。
const asVolatile = (schema) => (typeof schema?.volatile === "function" ? schema.volatile() : schema);

// 插件的 Config：preset / providerName / maxDepth / spillAllFields / largeFieldLines 是
// 组合配置（cordis.patch.yml / profile patch 的非 volatile 字段）；stages / followupMode /
// compactionThresholdRatio / readWidenMinLines 是 .volatile() 字段——dsh 0.1.7 的 settings
// 服务据此生成表单，客户端用 ctx.configForms 读写，值持久化到 profile 的 Cordis patch
// （值持久化到 profile 的 Cordis patch）。业务侧统一经 config.<field>.get() 读取（见 apply）。
export const Config = Schema.object({
  ...
  stages: asVolatile(STAGES_SCHEMA),
  followupMode: asVolatile(FOLLOWUP_MODE_SCHEMA),
  compactionThresholdRatio: asVolatile(COMPACTION_THRESHOLD_SCHEMA),
  readWidenMinLines: asVolatile(READ_WIDEN_SCHEMA),
});
...
function stageConcurrencyReached(stage, running, starting, limit) {
  ...
  return new Error(
    ...
    + (runningCount === 0 && startingCount > 0
      ? "No child of this stage is actually running: the slot(s) are held by dispatch(es) still being created in the same program, so a settled notice will NOT free one — retry this target in a LATER step (once those creations have been admitted). "
      : "Wait for a settled notice (a child finishing frees its running slot), then retry this dispatch in a LATER step. ")
    + "There is NO cap on how many children a stage may CREATE in total, so the SAME workstream's later rounds still go to its existing child with pipeline_followup (which creates no child), while a NEW independent workstream gets its OWN fresh child as soon as a running slot frees. "
    + "Do NOT hand a new workstream to an unrelated child just to avoid the wait: that child carries its whole history into every later step, so cross-workstream reuse is the expensive option, not the cheap one. "
    + "If several independent workstreams are outstanding, re-dispatch them in a LATER step (one per workstream) as the running slots free up.",
  );
}
...
function hostActivationLimit(ctx) {
  const valid = (value) => (typeof value === "number" && Number.isFinite(value) && value >= 1 ? value : undefined);
  try {
    const settings = getService(ctx, "settings");
    // settings 是 SettingsForms：从 describe() 描述符里取 subagent 条目的值。
    if (typeof settings?.describe === "function") {
      const row = settings.describe({ redactSecrets: false }).find((entry) => entry.ns === "subagent");
      const value = valid(row?.value?.maxActiveSubagents);
...
function stageHostCapacityReached(stage, detail, limit) {
  ...
  return new Error(
    ...
    + `Host detail: ${detail}\\n`
    + "dsh 0.1.6-alpha.2+ caps how many continuable subagents may be ALIVE at once for this root session; every live child of every stage counts, and this capacity IS released when a child settles. "
...
/** read 窗口拓宽下限（0 = 关闭；缺省/非法回落出厂默认）。 */
function resolveReadWidenFloor(source) {
  return normalizeReadWidenFloor(source()?.readWidenMinLines);
}
...
function buildResultToolDef(ctx) {
  return {
    ...
    async execute(args, exec) {
      ...
      const target = await resolveFollowupTarget(ctx, getService(ctx, "subagents"), query, parent.id, exec.signal);
...
function buildStageToolDef(ctx, stage, source, cfg) {
  return {
    ...
    async execute(args, exec) {
      ...
      // files 是派发契约的一部分：允许显式声明「没有候选清单」（单个 -），但不允许静默
      // 省略——省略时子代理就从零重新发现，那正是「20 步读 48 个文件」的成因。
      const filesValue = args.files;
      if (typeof filesValue !== "string" || filesValue.trim().length === 0) {
        throw new Error(
          `${stage.toolName}: "files" is required — pass the NEWLINE-SEPARATED paths this stage must read (the orchestrator's already-opened/shortlisted set; for impl the workstream's owned files; for review the changed files). If you genuinely have no candidate list, pass the single character "-" and the subagent will do its own reconnaissance in ONE batch.`,
        );
      }
      ...
      const { fullPrompt } = await materializeDispatchMessage(stage, args, promptText, cfg, resolveReadWidenFloor(source));
      ...
      const presets = getService(ctx, "agentPresets");
      ...
      if (typeof subagents?.startContinuable !== "function") {
        throw stageUnavailable(
          stage,
          'subagent service is unavailable: subagents.startContinuable is not a function on the host subagents service — this host version cannot dispatch continuable stage subagents',
        );
      }
      // ── 每阶段并发上限准入 ────────────────────────────────────────────────
...
async function countRunningStageChildren(ctx, subagents, parentId, stageKey, signal) {
  ...
  if (rows !== undefined) {
    ...
    for (const row of rows ?? []) {
      ...
      if (stageOfChildRow(ctx, childId, row.label) !== stageKey) continue;
...
function interruptStageChild(ctx, entry, reason) {
  ...
  const parent = getService(ctx, "agents")?.get(entry.parentId);
  if (!parent) {
    entry.phase = "lost";
    ctx.logger.warn(
 ...
 * （base bundle 的 compaction-basic），所以直接取名为 "compaction" 的服务 **会成功返回那个错误的
...
async function compactFollowupTarget(ctx, childId, signal) {
  ...
  if (!child) {
    throw new Error(
      ...
      + "This is NOT a stage failure and NOT the UNAVAILABLE case — do not stop the task over it. Deliver without compacting (compact: false) and accept the interference, or dispatch a fresh child for a genuinely NEW workstream, and say out loud which one you chose.",
    );
  }
  const compaction = getService(ctx, "agentPresets")?.serviceFor?.(child, "compaction");
...
function buildFollowupToolDef(ctx, source, cfg) {
  return {
    ...
    description:
      ...
      "CONTRACT: params = child (required) + message (required unless issues[] is given) + files (REQUIRED — newline-separated paths this round must read; \"-\" declares none) + issues? + changeSet? + compact?. " +
      "FILES IS NOT OPTIONAL: pass the REAL list of every path this round must read (the plugin renders it into a one-program read instruction); a child whose target file set changed otherwise re-derives the whole list step by step (an observed reuse burned 7 steps / 15 reads doing exactly that). For a small correction inside the workstream the child is already working on, pass the single character \"-\". " +
      "Prefer this over dispatching a new stage when the user updates a requirement mid-flight. " +
      "A FOLLOWUP CREATES NO CHILD: it is never gated by the stage's running cap, and a message to a child that is still LIVE needs no new host slot. " +
    ...
    parameters: {
      ...
      properties: {
        ...
        files: {
          type: "string",
          description: "REQUIRED. The paths THIS round must read, newline-separated — the plugin renders them into a \"read every path in ONE run_code program\" instruction, exactly like a stage dispatch. Give the real list of every path this round must read (the plugin renders it into a one-program read instruction). For a small correction inside files the child is already working on, pass the single character \"-\" to declare that no candidate list applies.",
        },
        issues: {
          type: "array",
          items: ISSUE_SCHEMA,
    ...
    async execute(args, exec) {
      ...
      // files 是**复用路径**上的派发契约。省略时子代理会把整份清单重新发现一遍——
      // 实测一个被复用去干另一个 workstream 的 impl 在开头花了 7 步 / 15 次 read。
      // 允许显式声明「没有清单」（单个 -），但不允许静默省略。
      const filesValue = args.files;
      if (typeof filesValue !== "string" || filesValue.trim().length === 0) {
        throw new Error(
          "pipeline_followup: \"files\" is required — pass the NEWLINE-SEPARATED paths this round must read: the REAL list when the round touches files the child has not been working on, or the single character \"-\" for a small correction inside the workstream it is already working on. Omitting it makes the child re-derive the list step by step.",
        );
      }
      ...
      const deliveredText = [
        ...
        renderFilesBlock(filesValue, resolveReadWidenFloor(source)),
        issueList.length > 0
      ...
      const target = await resolveFollowupTarget(ctx, getService(ctx, "subagents"), args.child, parent.id, exec.signal);
      if (target.childId === undefined) {
...
async function queueFollowupMessage(subagents, parent, childId, text, signal) {
  ...
  if (typeof subagents?.prompt !== "function") {
    throw new Error("pipeline_followup: queue delivery unavailable — subagents.prompt is not a function on the host subagents service (host does not support the human-queue channel)");
  }
  const base = {
...
export async function apply(ctx, config = {}) {
  ...
  // ── 宿主面记账（安全 + 可观测）──────────────────────────────────────────────
  // 注入健康：matched = 组合了本预设的 ROOT 代理数，injected = 真的注册成功的数，
  // failed = 注入过程抛错的数。/status 与启动自检读它——「工具没出现」从此可观测，
  // 不必翻日志。
  const injection = { matched: 0, injected: 0, failed: 0 };

  // 所有宿主事件注册都走 onHost 包装：宿主改名/移除该事件时只记一条 WARN 并继续，
  // 绝不让 apply（或宿主的 boot）因为一个事件注册失败而中断。
  const registerHostEvent = (carrier, event, handler, options) => {
    const receipt = onHost(carrier, event, handler, options);
    if (!receipt.ok) {
      ctx.logger.warn(
        `[dsh-code-pipeline] host event "${event}" could not be registered: ${receipt.error?.message ?? String(receipt.error)} — dsh may have renamed or removed it, so the feature it drives is disabled (see README「兼容与自检」)`,
      );
    }
    return receipt.ok;
  };
  ...
  // 设置来源：dsh 0.1.7 起 Config 的可编辑字段是 .volatile() 引用，每次调用读
  // config.<field>.get() 即得最新值（settings 服务把表单写入 profile patch，loader
  // 热更新引用）。0.1.6 及更早没有 volatile 引用——本版本只支持 engines 声明的
  // >= 0.1.7-alpha.1，缺引用时按「设置不可用」处理：工具照常注册、设置回落出厂默认
  // （绝不让 apply 抛错，也绝不退回已删除的 installSection 通道）。
  const hasVolatileRef = (value) => value !== null && typeof value === "object" && typeof value.get === "function";
  const volatileReady = ["stages", "followupMode", "compactionThresholdRatio", "readWidenMinLines"]
    .every((field) => hasVolatileRef(config[field]));
  if (!volatileReady) {
    ctx.logger.warn(
      "[dsh-code-pipeline] this plugin requires dsh >= 0.1.7-alpha.1 (volatile Config fields); the host did not provide them — stage settings fall back to the built-in defaults and the settings card cannot persist values",
    );
  }
  const readVolatile = (value, fallback) => {
    try {
      if (hasVolatileRef(value)) {
        const current = value.get();
        return current === undefined ? fallback : current;
      }
      return value === undefined ? fallback : value;
    } catch (error) {
      // 读设置绝不抛进调用方：它跑在工具调用与 agent/request 路径上。
      ctx.logger.warn(`[dsh-code-pipeline] reading a volatile setting failed: ${error?.message ?? String(error)}`);
      return fallback;
    }
  };
  const readSettings = () => ({
    stages: readVolatile(config.stages, undefined),
    followupMode: readVolatile(config.followupMode, undefined),
    compactionThresholdRatio: readVolatile(config.compactionThresholdRatio, undefined),
    readWidenMinLines: readVolatile(config.readWidenMinLines, undefined),
  });
  let source = readSettings;

  // 压缩触发比例 → profile 的 preset-<preset> 声明行（经 configEditor）。写入只对
  // 「之后挂载」的代理生效，所以每次设置变化都重新对账（幂等：值没变不写盘）。
  const syncCompactionThreshold = () => {
    // 显式 catch：这条链在 apply / 设置变更时**后台**跑，未处理的 rejection 在
    // 新宿主上会直接让进程退出——写宿主 profile patch 的失败必须只留一条日志。
    Promise.resolve(reconcileCompaction(ctx, preset, source()?.compactionThresholdRatio)).catch((error) => {
      ctx.logger.warn(`[dsh-code-pipeline] compaction threshold sync failed: ${error?.message ?? String(error)}`);
    });
  };

  ctx.inject(["settings"], (settingsCtx) => {
    // volatile 改动即时反映到工具调用（source 每次读引用）；压缩阈值另需防抖写回 preset
    // 声明。防抖也避开与 settings 写入（configEditor.edit -> loader 热更新）的重入。
    let scheduled = false;
    const schedule = () => {
      if (scheduled) return;
      scheduled = true;
    ...
    if (typeof settingsCtx?.effect === "function") {
      settingsCtx.effect(() => registerHostEvent(settingsCtx, "loader/volatile-update", () => schedule()), "code-pipeline: compaction reconcile");
    }
    syncCompactionThreshold();
  });
  ...
  const matchedAgents = new WeakSet();
  let rootCheckWarned = false;

  const isRootAgent = (agent) => {
    const agents = getService(ctx, "agents");
    if (agents === undefined || typeof agents.roots !== "function" || typeof agents.isOwnedBy !== "function") {
      // 无法判定归属时**不注入**：子代理继承父代理的预设，一旦判错就会把阶段工具
      // 注入子代理的自有层（宿主 tools.restrict 不过滤自有层），比漏注入危险得多。
      if (!rootCheckWarned) {
        rootCheckWarned = true;
        ctx.logger.warn(
          `[dsh-code-pipeline] cannot verify ROOT status (agents.roots/isOwnedBy unavailable) — skipping agent injection rather than risking stage-tool leakage into a subagent (host API drift?; see README「兼容与自检」)`,
        );
      }
      return false;
    }
  ...
  const injectInto = (agent) => {
    ...
    try {
      ...
      if (!matchedAgents.has(agent)) {
        matchedAgents.add(agent);
        injection.matched += 1;
      }
      if (!isRootAgent(agent)) return;
      const subagents = getService(ctx, "subagents");
      if (!subagents || typeof subagents.getProvider !== "function" || subagents.getProvider(providerName) === undefined) {
        pending.add(agent);
        return;
      }
      const tools = getService(agent.ctx, "tools");
      if (tools === undefined || typeof tools.register !== "function") {
        injection.failed += 1;
        ctx.logger.warn(
          `[dsh-code-pipeline] cannot inject stage tools into agent "${agent.id}": the agent's tools service is unavailable (host API drift?; see README「兼容与自检」)`,
        );
        return;
      }
      for (const stage of STAGES) {
  ...
  registerHostEvent(ctx, "agent-preset/selected", (sessionId, selectedPreset) => {
    if (selectedPreset !== preset) return;
    const agent = getService(ctx, "agents")?.get(String(sessionId));
    if (!agent) return;
    try {
      injectInto(agent);
    } catch (error) {
  ...
  // 这里只做注入检查，不修改请求配置。
  registerHostEvent(ctx, "agent/request", async ({ agent }, next) => {
  ...
  registerHostEvent(ctx, "subagent/provider-added", (provider) => {
  ...
  registerHostEvent(ctx, "subagent/start", (info) => {
    // 跑在宿主的激活路径上：任何异常只告警，绝不向上抛。
    try {
      ...
      ctx.logger.warn(`[dsh-code-pipeline] subagent/start handling failed: ${error?.message ?? String(error)}`);
    }
  }, { global: true });

  registerHostEvent(ctx, "subagent/end", (info) => {
    // 整个处理器包在 try/catch 里：它跑在宿主的子代理 settle 路径上，抛错会打断
    // 完成通知甚至子代理收尾。解析/校验失败只是「回执不可用」，记账（释放名额、
    // 推进相位）必须在任何情况下照常完成。
    const childId = String(info?.id ?? "");
    const entry = childId === "" ? undefined : dispatched.get(childId);
    if (!entry) return;
    ...
    try {
      ...
      entry.result = { parsed: false, reason: "the envelope could not be processed: " + (error?.message ?? String(error)) };
      ctx.logger.warn(`[dsh-code-pipeline] envelope handling failed for ${entry.stage} subagent ${childId}: ${error?.message ?? String(error)}`);
    }
    try {
      ...
      ctx.logger.warn(`[dsh-code-pipeline] subagent/end bookkeeping failed for ${childId}: ${error?.message ?? String(error)}`);
    }
  }, { global: true });

  registerHostEvent(ctx, "agent/disposed", ({ agent }) => {
    // 跑在宿主的销毁路径上：任何异常只告警，绝不向上抛。
    try {
      ...
      ctx.logger.warn(`[dsh-code-pipeline] agent/disposed handling failed: ${error?.message ?? String(error)}`);
    }
  }, { global: true });
  ...
  // ⚠️ 这个监听器在 next() 之后还要读设置（volatile 引用）：任何异常都会抛进宿主的
  // 模型请求链，让整轮会话失败。读取与改写因此必须在 try/catch 内，失败时返回**原样
  // config**（绝不影响宿主请求）。
  registerHostEvent(ctx, "agent/request", async ({ agent }, next) => {
    let stage;
    try {
      stage = STAGES.find((candidate) => candidate.key === agent?.options?.stageKey);
    } catch {
      stage = undefined;
    }
    ...
    try {
      ...
      ctx.logger.warn(
        `[dsh-code-pipeline] reasoningEffort injection skipped for stage "${stage.key}": ${error?.message ?? String(error)}`,
      );
      return config;
    }
  });
  ...
  // READ_TOOL_MAX_LINES 用模块级常量（与 read 拓宽共用同一个上限口径）。
  ...
  // agent → Map<programId, string[]>：同一个 run_code 程序里已触发提醒的文件路径。
  // 一次程序里重读十几个文件是真会发生的（实测一次 23 个）：逐个挂一条 user 消息会让
  // 「提醒」本身变成上下文洪水（约 11KB），所以攒到程序结束（外层 run_code 的
  // post-execute）再合成一条。
  const pendingReadNotices = new WeakMap();
  ...
  /** 组装一条 read-hygiene 提醒。单文件用「这是第 N 次读」的措辞，多文件用汇总措辞。 */
  function readNotice(paths, detail) {
    const head = detail !== undefined && paths.length === 1
      ? "[read-hygiene] This is read #" + String(detail.count) + " of " + paths[0]
        + " across separate steps (now lines " + detail.start + "-" + detail.end + "). "
        + (detail.reread
          ? "You already have these lines in context. "
          : "You are paging through this file one small window at a time. ")
      : "[read-hygiene] This step re-read " + String(paths.length) + " file(s) you already had in context across separate steps: "
        + paths.slice(0, 8).map((path) => "`" + path + "`").join(", ")
        + (paths.length > 8 ? ", … (" + String(paths.length - 8) + " more)" : "")
        + ". ";
    return {
      id: randomUUID(),
      role: "user",
      content: [{ type: "text", text: head + "One extra step re-sends the ENTIRE context, so fetch every region you need for a file in ONE program (parallel tools.read calls), and never come back to a range you already have. Re-reads inside a single program are fine and never trigger this notice." }],
      source: { kind: "dsh-code-pipeline", form: "notice", summary: "read-hygiene" },
    };
  }

  /**
   * 记一次成功的 read，并判定是否该提醒。root 级 read 直接返回单文件提醒；嵌套 read
   * （run_code 内部）只把路径攒进 pendingReadNotices，等外层调用结束时合成一条。
   */
  function recordRead(agent, exec, window) {
    ...
    entry.nudged = true;
    const nested = exec?.rootCallId !== undefined && exec.rootCallId !== exec.callId;
    if (nested) {
      let bucket = pendingReadNotices.get(agent);
      if (bucket === undefined) { bucket = new Map(); pendingReadNotices.set(agent, bucket); }
      if (bucket.size > 64 && !bucket.has(programId)) bucket.clear(); // 只做有界兜底
      const list = bucket.get(programId) ?? [];
      list.push({ path: window.path, count: previous.length + 1, start: window.start, end: window.end, reread });
      bucket.set(programId, list);
      return undefined;
    }
    return readNotice([window.path], { count: previous.length + 1, start: window.start, end: window.end, reread });
  }

  /** 取出并清空本 agent 攒下的重读文件名（外层调用结束时调用）。 */
  function drainPendingReadNotices(agent) {
    const bucket = pendingReadNotices.get(agent);
    if (bucket === undefined || bucket.size === 0) return undefined;
    const seen = [];
    for (const list of bucket.values()) {
      for (const item of list) if (!seen.some((candidate) => candidate.path === item.path)) seen.push(item);
    }
    bucket.clear();
    if (seen.length === 0) return undefined;
    // 只有一个文件时沿用单文件措辞（含它的读次/窗口），多文件才用汇总措辞。
    if (seen.length === 1) return readNotice([seen[0].path], seen[0]);
    return readNotice(seen.map((item) => item.path), undefined);
  }

  /**
   * 观察一次工具调用。同一文件在**不同步骤**里被重复读（窗口重叠，或连续的小窗口分页）
   * 时产出提醒；同一次 run_code 程序内的多次 read 是鼓励的批量化，不提醒。
   *
   * 两条硬规则（0.3.7 的实测教训）：
   * - **失败的 read 不算读过**：`limit` 超上限、文件不存在、被策略拒绝的调用什么都没进
   *   上下文；把它们记进历史，紧接着的**重试**就会被误报成重读。真实会话里一次
   *   `limit: 2500` 的批量读失败了 25 个文件，下一步 `limit: 2000` 重读就触发 23 条提醒。
   * - **一次程序只出一条**：嵌套 read 的提醒攒到外层调用结束再合成，避免上面那种洪水。
   */
  function observeIncrementalRead(exec, result) {
    const agent = exec?.agent;
    if (agent === null || agent === undefined) return undefined;
    // root 级调用：rootCallId 缺省或等于自身 callId（宿主对根调用总是解析成同一个 id）。
    const unrooted = exec?.rootCallId === undefined || exec.rootCallId === exec.callId;
    if (exec?.name === "read") {
      // 失败的 read 既不记历史、也不提醒；顺手把上一步攒下的桶取走，避免泄漏。
      if (result?.isError === true) return unrooted ? drainPendingReadNotices(agent) : undefined;
      const window = readWindow(exec.name, exec.arguments);
      if (window === undefined) return undefined;
      const notice = recordRead(agent, exec, window);
      // 嵌套 read 不发提醒（同程序里还有别的 read 要攒）；root read 直接发。
      return notice ?? (unrooted ? drainPendingReadNotices(agent) : undefined);
    }
    return unrooted ? drainPendingReadNotices(agent) : undefined;
  }

  registerHostEvent(ctx, "tools/post-execute", async (exec, result, next) => {
    let reminder;
    try {
      reminder = observeIncrementalRead(exec, result);
    } catch {
  ...
  // ── read 读取窗口拓宽：tools/execute around-waterfall（执行前就地改写参数）────
  // 宿主把每次工具派发包成 ctx.waterfall(carrier, "tools/execute", mutableExec,
  // () => dispatchToolBody(mutableExec))——exec 是共享可变对象，在 next() 之前改写
  // exec.arguments，工具本体就用拓宽后的参数执行（官方 around-dispatch 扩展点；
  // 读取发生在 run_code 程序内部时同样流经这里）。只对本插件的读者生效：阶段子代理
  //（派发时打在 agentOptions 上的 stageKey 标记）+ 组合了本预设的 root；其它会话
  // 的代理不受影响。post-execute 的读卫生观察看到的已是拓宽后的参数（同一个 exec
  // 对象），因此拓宽过的读按满窗记账，不会被误判成「小窗口分页」。全程 try/catch：
  // 拓宽失败绝不阻塞读取本体，退回原始参数继续执行。
  const stageKeySet = new Set(STAGES.map((stage) => stage.key));
  const isPipelineReader = (agent) => {
    if (agent === null || agent === undefined) return false;
    if (stageKeySet.has(agent.options?.stageKey)) return true;
    try {
      return getService(ctx, "agentPresets")?.composedPreset(agent.ctx) === preset;
    } catch {
      return false;
    }
  };
  registerHostEvent(ctx, "tools/execute", async (exec, next) => {
    try {
      if (exec?.name === "read") {
        const floor = resolveReadWidenFloor(source);
        if (floor > 0 && isPipelineReader(exec.agent)) {
          const widened = widenReadArguments(exec.arguments, floor);
          if (widened !== undefined) {
            exec.arguments = widened.arguments;
            ctx.logger.info(
              `[dsh-code-pipeline] read window widened for agent ${exec.agent?.id}: ${widened.arguments.file_path} limit ${widened.from} -> ${widened.to}${widened.healed ? " (over-cap healed)" : ""}`,
            );
          }
        }
      }
    } catch {
      /* 拓宽失败绝不阻塞读取：保持原始参数继续执行 */
    }
    return next();
  }, { global: true });
  ...
  // 设置页的并发状态端：各阶段当前运行数 / 启动中 / 上限 / 已创建数 / 已创建清单
  //（只读，跨父会话汇总）。
  const statusHandler = async (_req, res) => {
    ...
    for (const stage of STAGES) {
      // 并发是**按会话独立**判定的：每个父会话各自拿同一个 limit 去比对，互不占用对方的名额。
      // 设置页是一张全局卡片（不知道要看哪个会话），所以如实报告「单会话最多」（running/pending，
      // 也就是真正与 limit 比较的那个量）与「共几个会话 / 合计」（sessions/totalRunning/totalPending，
      // 仅供诊断）。**绝不**把跨会话的运行数相加后当成 running —— 那会把「每个会话独立」误显示成
      // 「所有会话合计」。
      let running = 0;
      let pendingCount = 0;
      let totalRunning = 0;
      let totalPending = 0;
      let sessions = 0;
      const sessionIds = new Set([...activeStageChildren.keys(), ...pendingStageStarts.keys()]);
      for (const parentId of sessionIds) {
        const sessionRunning = activeStageCount(parentId, stage.key);
        const sessionPending = pendingStageCount(parentId, stage.key);
        if (sessionRunning + sessionPending === 0) continue;
        sessions += 1;
        totalRunning += sessionRunning;
        totalPending += sessionPending;
        if (sessionRunning > running) running = sessionRunning;
        if (sessionPending > pendingCount) pendingCount = sessionPending;
      ...
      // created = 本进程台账 ∪ 宿主当前可见行（信息展示：本会话该阶段创建过的子代理）；
      // available = 这份并集本身（元素 { id, label, activity }），仅作展示。
      ...
      stages[stage.key] = {
        // running / pending = 单会话最多（与 limit 同口径，按会话独立）；total* = 跨会话合计（仅信息）。
        running,
    ...
    // 宿主自检 + 注入健康：设置页与排障只需要看这一处，不必翻日志。
    // 每次轮询**实时**探测（服务可能在插件 apply 之后才注册）；探测本身绝不抛错。
    let hostReport;
    try {
      const checks = probeHost(ctx);
      hostReport = {
        checks,
        broken: describeHostChecks(checks),
        injection: { ...injection },
      };
    } catch (error) {
      hostReport = { checks: [], broken: [], injection: { ...injection }, error: error?.message ?? String(error) };
    }
  ...
  const registerRoutes = () => {
    const server = getService(ctx, "webServer");
  ...
  if (!registerRoutes()) {
    ...
    retry.unref?.(); // 不阻止进程退出（测试/短命宿主进程不挂在它上面）
  ...
  // ── 宿主自检（启动快照）─────────────────────────────────────────────────────
  // 把「dsh 改名/移除某个宿主面 → 插件静默失效」变成可诊断的日志。探测绝不抛错。
  // 注意：服务可能在插件 apply **之后**才注册（本插件本来就有 provider-added 等补注入
  // 路径），所以启动时的「缺席」只记 INFO；只有**在场但方法形状不对**才是 WARN
  // （那才是真正的宿主 API 漂移）。查询期间的实时结果由 /status 的 host 字段给出。
  try {
    const checks = probeHost(ctx);
    const summary = checks
      .map((check) => `${check.id}=${check.ok === false ? "BROKEN" : check.present === false ? "absent" : "ok"}`)
      .join(" ");
    ctx.logger.info(`[dsh-code-pipeline] host self-check (${HOST_CONTRACT.length} surfaces): ${summary}`);
    for (const check of checks) {
      if (check.present === false) {
        ctx.logger.info(
          `[dsh-code-pipeline] host surface "${check.id}" (${check.required ? "required" : "optional"}) is not registered at apply time — it may register later; /dsh-code-pipeline/status reports the live state`,
        );
        continue;
      }
      if (check.missingMethods.length > 0) {
        ctx.logger.warn(
          `[dsh-code-pipeline] host surface "${check.id}" is present but missing required method(s): ${check.missingMethods.join(", ")} — dsh changed its shape; the features using them are disabled (see README「兼容与自检」)`,
        );
      }
      if (check.degradedMethods.length > 0) {
        ctx.logger.info(
          `[dsh-code-pipeline] host surface "${check.id}" lacks optional method(s): ${check.degradedMethods.join(", ")} — the corresponding feature degrades (documented fallback)`,
        );
      }
    }
  } catch (error) {
    ctx.logger.warn(`[dsh-code-pipeline] host self-check failed: ${error?.message ?? String(error)}`);
  }
}