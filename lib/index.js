...
export async function apply(ctx, config = {}) {
  ...
  // 只有在「小窗口」上开火（limit < 工具上限）：文件超过工具上限时分块读是被迫的，
  // 不算浪费。
  const READ_TOOL_MAX_LINES = 2000;
  const lastReads = new WeakMap(); // agent → { path, start, end, limit }
  const nudgedReads = new WeakMap(); // agent → Set<path>（同一文件只提醒一次）
  ...
  /**
   * 观察一次 read，判断它是不是「同一文件的分块续读」，是则返回要挂上的提醒消息。
   * 两个窗口必须重叠或首尾相接（明显有间隔的两次读不算）；两个窗口都必须小于
   * 工具上限，否则分块是被工具本身逼出来的。
   */
  function observeIncrementalRead(exec) {
    ...
    const previous = lastReads.get(agent);
    lastReads.set(agent, window);
    if (previous === undefined || previous.path !== window.path) return undefined;
    if (previous.limit >= READ_TOOL_MAX_LINES || window.limit >= READ_TOOL_MAX_LINES) return undefined;
    const touching = window.start <= previous.end + 1 && previous.start <= window.end + 1;
    if (!touching) return undefined;
    let nudged = nudgedReads.get(agent);
    if (nudged === undefined) { nudged = new Set(); nudgedReads.set(agent, nudged); }
    if (nudged.has(window.path)) return undefined;
    nudged.add(window.path);
    const text =
      "[read-hygiene] You are reading " + window.path + " in pieces (lines " + previous.start + "-" + previous.end
      + ", now " + window.start + "-" + window.end + "). "
      + "One extra step re-sends the ENTIRE context, so a single wide read is cheaper than a second round trip: "
      + "read the whole file (or the widest range that could matter) now, and never re-read a range you already have.";
    return {
      id: randomUUID(),
      role: "user",
      content: [{ type: "text", text }],
