  ...
  return {
    ...
    parameters: {
      ...
      run_in_background: {
        type: "boolean",
        description:
          "FIXED: stages are background-only — the call always returns a durable subagentId immediately and the runtime notifies this session when it settles (track status with list_agents, fetch results with subagent.history). Passing false is rejected: never wait for a stage inside the current turn (run_code is capped at a 20-minute wall clock). Very short tasks should NOT be dispatched at all — nothing to do here.",
      },
    },
    output: {
      schema: {
