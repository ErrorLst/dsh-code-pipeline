...
//
// 默认下限 500（0.4.5 曾为 2000，实测反馈后调低）：本预设所有代理都是 PTC
// （Code Mode），read 只在 run_code 程序内部发生——嵌套工具结果**只进程序、不进
// 模型历史**，拓宽本身零 token 成本；500 已覆盖典型源码文件的大半，把「几十行
// 几十行翻」的步数砍掉一个数量级，又不对超大文件一次拉满。想「一律整窗」把
// 设置项调到 2000（= 工具上限），置 0 关闭。已保存过旧值的安装显式存储优先于
// 默认，需在设置页重存一次。结果自带行号与 totalLines，窗口被拓宽是自描述的。

/** read 工具的行数上限（缺省 limit 也是它）：与宿主 dsh-tool-fs 的 caps.limit 一致。 */
export const READ_TOOL_MAX_LINES = 2000;

/** readWidenMinLines 的出厂默认（0.4.6 起 500）：覆盖典型源码文件大半，又不至于对超大文件一次拉满；2000 = 一律整窗。 */
export const READ_WIDEN_DEFAULT_LINES = 500;
 ...
 */
function renderFilesBlock(value) {
  ...
  return "**files** — "
    ...
    + (declaredNone ? "" : "\n" + value)
    + " Read whole files by OMITTING `limit` — `tools.read` caps it at 2000 lines (the plugin heals an over-cap `limit` down to 2000 and auto-widens a small one up to the window floor, so a batch no longer fails on `limit` — but OMIT it anyway: that is the whole-window default). Build the argument object by OMITTING optional keys too: a key whose value is `undefined` (`{ include: undefined }`) is rejected before dispatch as non-lossless JSON.";
}
...
const READ_WIDEN_SCHEMA = Schema.number()
  ...
  .description("read 读取窗口下限（行）：本预设代理（主会话与阶段子代理）发出的 read，请求的 limit 低于该值时会在执行前被就地拓宽到该值——消除「一个文件几十行几十行地翻、每多一步重发整个上下文」的分页模式（嵌套 read 的结果只进 run_code 程序、不进模型历史，拓宽本身零 token 成本，一个程序即可拿足上下文）。超过工具上限 2000 的 limit 会被治愈为 2000，不再整批失败。默认 500；2000 = 一律整窗（等价于省略 limit）；0 = 关闭拓宽。改动立即对后续 read 生效，无需重启。");
