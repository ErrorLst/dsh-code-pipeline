...
//   compactionThresholdRatio) through the configForms service
...
//
// maxConcurrency 是「同时运行」的并发上限（宿主 lib/index.js 一侧执行，本卡片只负责
// 如实展示）：同一父会话内该阶段最多同时跑几个子代理，0 = 不限制。**按会话独立判定**——
// 多个会话各自拿同一个 limit 计数，互不占用名额；status 的 running/pending 是「单会话
// 最多」（与 limit 同口径），totalRunning/totalPending/sessions 是跨会话合计（仅诊断）。
// 0.4.0 起不再有「已创建总量」上限——新工作流一律派发新子代理；只有同一工作流的后续轮次才用
// pipeline_followup 续用同一个孩子。
// status.stages[key].created（全部会话创建过的数量）与 status.stages[key].available
//（已创建子代理数组，元素 { id, label, activity }）由宿主并行开发：两者都可能缺失，
// 缺失时静默降级（不显示该半句、不渲染 undefined）。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // read 读取窗口下限（readWidenMinLines）的客户端兜底：与宿主 SettingsSchema 同默认（0.4.7 起 200）。
    const READ_WIDEN_FALLBACK = 200;
    const READ_WIDEN_MIN = 0;
    const READ_WIDEN_MAX = 2000;
    const READ_WIDEN_HINT =
      "read 读取窗口下限（行）：本预设代理发出的 read，请求的 limit 低于该值时会在执行前被就地拓宽到该值（超过 2000 的治愈为 2000，不再整批失败）。嵌套 read 的结果只进 run_code 程序、不进模型历史——拓宽零 token 成本，一个程序即可拿足上下文，从机制上消除『一个文件几十行几十行地翻、每多一步重发整个上下文』的分页模式。默认 200；2000 = 一律整窗（等价于省略 limit）；0 = 关闭。改动立即对后续 read 生效。";
    ...
    function StageCard({ row, stage, options, status, onChange }) {
      ...
      const concurrencyStatus =
        "当前运行（单会话最多）" + liveRunning + (livePending > 0 ? "（启动中 " + livePending + "）" : "")
        + " / 单会话上限 " + (limit > 0 ? limit : "不限制")
      ...
      const reusableHint = reusableLabels.length === 0
        ? ""
        : "全部会话已创建的子代理（同一工作流的后续轮次用 pipeline_followup 续用）："
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement("label", { style: LABEL_STYLE }, "同一父会话内该阶段允许「同时在跑」的子代理数上限——按会话独立判定，多个会话各自计数、互不占用对方名额。没有「已创建总量」上限：新工作流一律派发新子代理，同一工作流的后续轮次才用 pipeline_followup 续用；到达运行上限时新的派发会被拒绝，等有子代理结束（名额释放）后在后续步骤重新派发即可。0 = 不限制。仍受宿主每个 run_code 程序最多 10 个并行子调用的上限约束。"),
    ...
    function PipelineSection(props) {
      ...
      return React.createElement(
        ...
        React.createElement(
          "p",
          { style: INTRO_STYLE },
          "配置各阶段子代理的模型、并发上限与墙钟预算；保存后对下一次阶段派发生效，无需重启；阶段关闭后其工具调用会直接报错。最大并发按 (父会话 × 阶段) 限制「同时运行」的子代理数；没有「已创建总量」上限——新工作流一律新派，同一工作流的后续轮次用 pipeline_followup 续用。调低只拦后续派发、不中断在跑的子代理。墙钟预算 = 单次派发的最长运行时间；超时自动中断并索取收尾报告，供主代理决定续跑 / 拆分 / 停止。",
        ),
        ...STAGE_ROWS.map((row) =>
        ...
        React.createElement(
          ...
          React.createElement("label", { style: LABEL_STYLE }, READ_WIDEN_HINT),
        ),
        optionsError
