...
//   reasoningEffort / maxConcurrency / budgetMinutes) through the settingsScope
    ...
    function StageCard({ row, stage, options, status, onChange }) {
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement("label", { style: LABEL_STYLE }, "该阶段单次派发的最长运行时间；0 = 不限制。预算用到 80% 时先给运行中的子代理插一条「开始收尾」警告；到 100% 则中断该子代理（只结束当前回合，Activation 与 inbox 保留，之后仍可用 pipeline_followup 续跑），并自动排队一条收尾报告指令，让它汇报「已完成 / 半成品 / 未完成 / 风险 / 建议」，供主代理决定续跑、拆分还是停止。只对之后的派发生效。"),
    ...
    function PipelineSection(props) {
      ...
      return React.createElement(
        ...
        React.createElement(
          "p",
          { style: INTRO_STYLE },
          "配置 code-pipeline 预设各阶段子代理的模型、并发上限与墙钟预算。保存后对下一次阶段派发生效（工具每次调用时读取设置，无需重启 dsh）。任一阶段关闭后，对应阶段工具调用会直接报错。最大并发 = 同一父会话内该阶段同时运行的子代理数上限（0 = 不限制）；调低不会中断正在运行的子代理，只拦截后续派发；独立目标可在一个程序里并行派发多个阶段子代理以加快进度。墙钟预算 = 该阶段单次派发的最长运行时间（0 = 不限制）；超时后自动中断该子代理并索取一份收尾报告，供主代理决定续跑 / 拆分 / 停止。",
        ),
        ...STAGE_ROWS.map((row) =>
