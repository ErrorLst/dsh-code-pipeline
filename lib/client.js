...
//   复用的子代理清单 —— 出路是用 pipeline_followup 复用已有子代理（该阶段子代理
//   已被结束时先 pipeline_followup({compact:true}) 压缩、再复用），而不是等待一个
//   跑完后再派新的。0 = 不限制。
    ...
    function PipelineSection(props) {
      ...
      return React.createElement(
        ...
        React.createElement(
          "p",
          { style: INTRO_STYLE },
          "配置 code-pipeline 预设各阶段子代理的模型、并发上限与墙钟预算。保存后对下一次阶段派发生效（工具每次调用时读取设置，无需重启 dsh）。任一阶段关闭后，对应阶段工具调用会直接报错。最大并发 = 同一父会话内该阶段同时运行的子代理数上限，同时也是该阶段「已创建（含已结束）」子代理的总量上限（0 = 不限制）；到达已创建总量上限后新的阶段派发会被拒绝，请用 pipeline_followup 复用已有子代理（有干扰时先压缩再复用），而不是等一个跑完再派新的；调低不会中断正在运行的子代理，只拦截后续派发；独立目标可在一个程序里并行派发多个阶段子代理以加快进度。墙钟预算 = 该阶段单次派发的最长运行时间（0 = 不限制）；超时后自动中断该子代理并索取一份收尾报告，供主代理决定续跑 / 拆分 / 停止。",
        ),
        ...STAGE_ROWS.map((row) =>
