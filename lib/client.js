    ...
    function StageCard({ row, stage, options, status, onChange }) {
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement("label", { style: LABEL_STYLE }, "该阶段单次派发的最长运行时间；0 = 不限制。超时后插件中断该子代理（只结束当前回合，Activation 与 inbox 保留，之后仍可用 pipeline_followup 续跑），并自动排队一条收尾报告指令，让它汇报「已完成 / 半成品 / 未完成 / 风险 / 建议」，供主代理决定续跑、拆分还是停止。只对之后的派发生效。"),
