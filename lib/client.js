...
//   through the settingsScope service; the host plugin's stage tools read the
//   namespace at every call, so saving takes effect immediately — the next
//   subagent dispatch uses the new model, no dsh restart needed.
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CLIENT_DEFAULT_STAGES = {
      plan: { enabled: true, provider: "deepseek-official", model: "deepseek-v4-flash", reasoningEffort: "" },
      impl: { enabled: true, provider: "deepseek-official", model: "deepseek-v4-flash", reasoningEffort: "" },
      review: { enabled: true, provider: "deepseek-official", model: "deepseek-v4-flash", reasoningEffort: "" },
    };
    ...
    function PipelineSection(props) {
      ...
      return React.createElement(
        ...
        React.createElement(
          "p",
          { style: INTRO_STYLE },
          "配置 code-pipeline 预设各阶段子代理的模型。保存后对下一次阶段派发生效（工具每次调用时读取设置，无需重启 dsh）。任一阶段关闭后，对应阶段工具调用会直接报错。",
        ),
        ...STAGE_ROWS.map((row) =>
