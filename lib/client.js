...
// 如实展示）：同一父会话内该阶段最多同时跑几个子代理，0 = 不限制。0.4.0 起不再有
// 「已创建总量」上限——新工作流一律派发新子代理；只有同一工作流的后续轮次才用
// pipeline_followup 续用同一个孩子。
// status.stages[key].created（本会话创建过的数量）与 status.stages[key].available
  ...
  factory: (require) => {
    ...
    function StageCard({ row, stage, options, status, onChange }) {
      ...
      const reusableHint = reusableLabels.length === 0
        ? ""
        : "本会话已创建的子代理（同一工作流的后续轮次用 pipeline_followup 续用）："
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement("label", { style: LABEL_STYLE }, "同一父会话内该阶段允许「同时在跑」的子代理数上限。没有「已创建总量」上限：新工作流一律派发新子代理，同一工作流的后续轮次才用 pipeline_followup 续用；到达运行上限时新的派发会被拒绝，等有子代理结束（名额释放）后在后续步骤重新派发即可。0 = 不限制。仍受宿主每个 run_code 程序最多 10 个并行子调用的上限约束。"),
