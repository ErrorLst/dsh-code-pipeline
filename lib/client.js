...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // read 读取窗口下限（readWidenMinLines）的客户端兜底：与宿主 SettingsSchema 同默认。
    const READ_WIDEN_FALLBACK = 2000;
    const READ_WIDEN_MIN = 0;
    const READ_WIDEN_MAX = 2000;
    const READ_WIDEN_HINT =
      "read 读取窗口下限（行）：本预设代理发出的 read，请求的 limit 低于该值时会在执行前被就地拓宽到该值（超过 2000 的治愈为 2000，不再整批失败）。嵌套 read 的结果只进 run_code 程序、不进模型历史——拓宽零 token 成本，一个程序即可拿足上下文，从机制上消除『一个文件几十行几十行地翻、每多一步重发整个上下文』的分页模式。默认 2000 = 一律整窗；0 = 关闭。改动立即对后续 read 生效。";
